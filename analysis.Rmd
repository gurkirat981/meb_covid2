...
ias_processed_dta<-ias_processed_dta %>% 
  ...
  mutate(sdgtier = ifelse(State == "Uttar Pradesh" | State == "Bihar" | State == "Assam", "Aspirant", 
                       ifelse(State=="Puducherry"|State=="Tamil Nadu"| State=="Chandigarh"| State=="Himachal Pradesh"|State =="Kerala", "Frontrunner", "Performer"))) %>% 
  mutate(sdgtier_weight = ifelse(sdgtier == "Aspirant", total/886, 
                             ifelse(sdgtier == "Frontrunner", total/629, total/3764)))
                             
#totals for sdg_tier(s) are manually caluculated: Aspirants: 886, Frontrunners: 629, Performers: 3764
#totals are subjected to change if the state distribution is altered
