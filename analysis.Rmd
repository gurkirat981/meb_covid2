...
summary_state_young<-fullage_young %>% 
  ...
  arrange(state) 
#There are no young officers from A and N Islands, Chandigarh, Lakshadweep Islands, Puducherry and Mizoram. The summary produced for young officers, thus, does not have rows for the following states. To match the number of rows with other datasets, we have added these states with NAs. This is done for all summaries for young and old officers. (cbind)>>>>>>
```
...
summary_state_old<-fullage_old %>% 
  ...
  arrange(state) 
#There are no young officers from A and N Islands, Chandigarh, Lakshadweep Islands, Puducherry and Mizoram. The summary produced for young officers, thus, does not have rows for the following states. To match the number of rows with other datasets, we have added these states with NAs. This is done for all summaries for young and old officers.
```
...
summary_q4_young <- state_answers_q4_young %>% 
  ...
  add_row(state="Mizoram") %>% 
  arrange(state) 
#We do not have any young officers from A and N Islands, Chandigarh, Lakshadweep Islands, Puducherry and Mizoram. Hence, the summary created for this question does not have rows for these states. To make it uniform with other graphs, we add these rows with NA values. 
...
\vspace{6mm}
```{r echo = FALSE, fig.height=10,fig.width=18}
q15_plotlabels <- c("Strongly disagree", "Slightly disagree", "Slightly agree", "Strongly agree")
