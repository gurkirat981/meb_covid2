...
#Merging the three above subsets to get the desired full_serving dataset with correct state allocations
...
#need to look for an alternative to divide combined cadres
ias_processed_dta<-ias_processed_dta %>% 
  ...
  mutate(State = ifelse(State== "Assam Meghalya", sample(c("Assam", "Meghalaya"), prob = c(0.69, 0.31), size = length(which(ias_processed_dta$State=="Assam Meghalya")), replace = TRUE),   
                       ifelse(State=="A G M U T", sample(c("Delhi", "Arunachal Pradesh","Mizoram","Goa", "DNH and DD","Chandigarh", "Andaman and Nicobar Islands", "Lakshadweep Islands", "Puducherry"), prob = c(0.36, 0.18, 0.14, 0.09, 0.05, 0.05,0.05, 0.05, 0.05), size = length(which(ias_processed_dta$State=="A G M U T")), replace = TRUE),
  ...
  mutate(prop_pre2012_withinstate = pre2012/sum(total)) %>% 
  mutate(prop_post2012_withinstate = post2012/sum(total)) 
  
  ias_processed_dta <- ias_processed_dta %>% 
  mutate(sdgtier = ifelse(State == "Uttar Pradesh" | State == "Bihar" | State == "Assam", "Aspirant", 
                       ifelse(State=="Puducherry"|State=="Tamil Nadu"| State=="Chandigarh"| State=="Himachal Pradesh"|State =="Kerala", "Frontrunner", "Performer"))) 
  
ias_processed_dta <- ias_processed_dta %>% 
  mutate(sdgtier_weight = ifelse(sdgtier == "Aspirant", total/sum(ias_processed_dta$total[ias_processed_dta$sdgtier=="Aspirant"]), 
                             ifelse(sdgtier == "Frontrunner", total/sum(ias_processed_dta$total[ias_processed_dta$sdgtier=="Frontrunner"]), total/sum(ias_processed_dta$total[ias_processed_dta$sdgtier=="Performer"]))))
  
  #age_weight_young is the proportion of young officers in X state out of all young officers. The sum of prop_pre2012_age for all states is equal to 1 
...
state_answers_q4 <- na.omit(state_answers_q4)

summary_q4 <- state_answers_q4 %>% 
  dplyr::select(state, Merged) %>% 
  group_by(state) %>% 
  summarise(q4_ANIslands_mean=sum(Merged =="Andaman and Nicobar Islands")/length(state),
            q4_AndhraPradesh_mean=sum(Merged =="Andhra Pradesh")/length(state), 
            q4_ArunachalPradesh_mean=sum(Merged =="Arunachal Pradesh")/length(state),
            q4_Assam_mean=sum(Merged =="Assam")/length(state),
            q4_Bihar_mean=sum(Merged =="Bihar")/length(state),
            q4_Chandigarh_mean=sum(Merged =="Chandigarh")/length(state),
            q4_Chattisgarh_mean=sum(Merged =="Chattisgarh")/length(state),
            q4_Delhi_mean=sum(Merged =="Delhi")/length(state),
            q4_DNHandDD_mean=sum(Merged =="DNH and DD")/length(state),
            q4_Goa_mean=sum(Merged =="Goa")/length(state),
            q4_Haryana_mean=sum(Merged =="Haryana")/length(state),
            q4_Gujrat_mean=sum(Merged =="Gujarat")/length(state),
            q4_HimachalPradesh_mean=sum(Merged =="Himachal Pradesh")/length(state),
            q4_JammuKashmir_mean=sum(Merged =="Jammu and Kashmir")/length(state),
            q4_Jharkhand_mean=sum(Merged =="Jharkhand")/length(state),
            q4_Karnataka_mean=sum(Merged =="Karnataka")/length(state),
            q4_Kerala_mean=sum(Merged =="Kerala")/length(state),
            q4_MadhyaPradesh_mean=sum(Merged =="Madhya Pradesh")/length(state),
            q4_LakshadweepIslands_mean=sum(Merged =="Lakshadweep Islands")/length(state),
            q4_Maharashtra_mean=sum(Merged =="Maharashtra")/length(state),
            q4_Manipur_mean=sum(Merged =="Manipur")/length(state),
            q4_Mizoram_mean=sum(Merged =="Mizoram")/length(state),
            q4_Meghalaya_mean=sum(Merged =="Meghalaya")/length(state),
            q4_Nagaland_mean=sum(Merged =="Nagaland")/length(state),
            q4_Odisha_mean=sum(Merged =="Odisha")/length(state),
            q4_Puducherry_mean=sum(Merged =="Puducherry")/length(state),
            q4_Punjab_mean=sum(Merged =="Punjab")/length(state),
            q4_Rajasthan_mean=sum(Merged =="Rajasthan")/length(state),
            q4_Sikkim_mean=sum(Merged =="Sikkim")/length(state),
            q4_TamilNadu_mean=sum(Merged =="Tamil Nadu")/length(state),
            q4_Telangana_mean=sum(Merged =="Telangana")/length(state),
            q4_Tripura_mean=sum(Merged =="Tripura")/length(state),
            q4_UttarPradesh_mean=sum(Merged =="Uttar Pradesh")/length(state),
            q4_Uttarakhand_mean=sum(Merged =="Uttarakhand")/length(state),
            q4_WestBengal_mean=sum(Merged =="West Bengal")/length(state)) %>% 
ungroup()
```
...
summary_15<- state_answers_q5 %>% 
  group_by(state) %>% 
  na.omit() %>% 
  summarise(ANIslands=sum(Merged=="Andaman and Nicobar Islands")/ length(state),
            AP=sum(Merged=="Andhra Pradesh")/length(state),
            Assam=sum(Merged=="Assam")/length(state),
            Bihar=sum(Merged=="Bihar")/length(state),
            Chhattisgarh=sum(Merged=="Chhattisgarh")/length(state),
            Goa=sum(Merged=="Goa")/length(state),
            Gujarat=sum(Merged=="Gujarat")/length(state),
            Haryana=sum(Merged=="Haryana")/length(state),
            HP=sum(Merged=="Himachal Pradesh")/length(state),
            JK=sum(Merged=="Jammu and Kashmir")/length(state),
            Jharkhand=sum(Merged=="Jharkhand")/length(state),
            Karnataka=sum(Merged=="Karnataka")/length(state),
            Kerala=sum(Merged=="Kerala")/length(state),
            Lakshadweep=sum(Merged=="Lakshadweep")/length(state),
            MP=sum(Merged=="Madhya Pradesh")/length(state),
            Maharashtra=sum(Merged=="Maharashtra")/length(state),
            Manipur=sum(Merged=="Manipur")/length(state),
            Meghalaya=sum(Merged=="Meghalaya")/length(state),
            Mizoram=sum(Merged=="Mizoram")/length(state),
            Delhi=sum(Merged=="NCT of Delhi")/length(state),
            None=sum(Merged=="No state has performed poorly")/length(state),
            Puducherry=sum(Merged=="Puducherry")/length(state),
            Punjab=sum(Merged=="Punjab")/length(state),
            Rajasthan=sum(Merged=="Rajasthan")/length(state),
            TN=sum(Merged=="Tamil Nadu")/length(state),
            Telangana=sum(Merged=="Telangana")/length(state),
            UP=sum(Merged=="Uttar Pradesh")/length(state),
            Uttarakhand=sum(Merged=="Uttarakhand")/length(state),
            WB=sum(Merged=="West Bengal")/length(state)) %>% 
  ungroup() %>% 
  mutate(total=rowSums(select_if(., is.numeric), na.rm = TRUE))
 

#Merging with summary_state_analysis
```
