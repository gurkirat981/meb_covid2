...
#Calculating averages for *unbiased* officers only (by filtering out all officers who selected their own state)
non_repeated_q4 <- state_answers_q4 %>% 
mutate (REPEAT = ifelse (Merged == "Andaman and Nicobar Islands " & X12 == "AGMUT", 1,
...
ifelse (Merged == "Chhattisgarh" & X12 == "Chattisgarh", 1,
...
ifelse (Merged == "Lakshadweep" & X12 == "Jammu and Kashmir", 1,
...
ifelse (Merged == "Mizoram" & X12 == "Andhra Pradesh", 1,
...
nr_q4 <- non_repeated_q4 %>% 
  filter (REPEAT == 0) 

summary_nr_q4 <- nr_q4 %>% 
  group_by(state) %>% 
  na.omit() %>% 
  summarise(q4_anislands_nr=sum(Merged =="Andaman and Nicobar Islands")/length(state),
            q4_andhra_nr=sum(Merged =="Andhra Pradesh")/length(state), 
            q4_arunachal_nr=sum(Merged =="Arunachal Pradesh")/length(state),
            q4_assam_nr=sum(Merged =="Assam")/length(state),
            q4_bihar_nr=sum(Merged =="Bihar")/length(state),
            q4_chandigarh_nr=sum(Merged =="Chandigarh")/length(state),
            q4_chhattisgarh_nr=sum(Merged =="Chhattisgarh")/length(state),
            q4_dnhdd_nr=sum(Merged =="Dadra and Nagar Haveli and Daman and Dui")/length(state),
            q4_goa_nr=sum(Merged =="Goa")/length(state),
            q4_gujarat_nr=sum(Merged =="Gujarat")/length(state),
            q4_haryana_nr=sum(Merged =="Haryana")/length(state),
            q4_hp_nr=sum(Merged =="Himachal Pradesh")/length(state),
            q4_jk_nr=sum(Merged =="Jammu and Kashmir")/length(state),
            q4_jharkhand_nr=sum(Merged =="Jharkhand")/length(state),
            q4_karnataka_nr=sum(Merged =="Karnataka")/length(state),
            q4_kerala_nr=sum(Merged =="Kerala")/length(state),
            q4_ladakh_nr=sum(Merged =="Ladakh")/length(state),
            q4_lakshadweep_nr=sum(Merged =="Lakshadweep")/length(state),
            q4_mp_nr=sum(Merged =="Madhya Pradesh")/length(state),
            q4_maharashtra_nr=sum(Merged =="Maharashtra")/length(state),
            q4_manipur_nr=sum(Merged =="Manipur")/length(state),
            q4_meghalaya_nr=sum(Merged =="Meghalaya")/length(state),
            q4_mizoram_nr=sum(Merged =="Mizoram")/length(state),
            q4_nagaland_nr=sum(Merged =="Nagaland")/length(state),
            q4_delhi_nr=sum(Merged =="NCT of Delhi")/length(state),
            q4_none_nr=sum(Merged =="No state has performed well")/length(state),
            q4_odisha_nr=sum(Merged =="Odisha")/length(state),
            q4_puducherry_nr=sum(Merged =="Puducherry")/length(state),
            q4_punjab_nr=sum(Merged =="Punjab")/length(state),
            q4_rajasthan_nr=sum(Merged =="Rajasthan")/length(state),
            q4_sikkim_nr=sum(Merged =="Sikkim")/length(state),
            q4_tn_nr=sum(Merged =="Tamil Nadu")/length(state),
            q4_telangana_nr=sum(Merged =="Telangana")/length(state),
            q4_tripura_nr=sum(Merged =="Tripura")/length(state),
            q4_up_nr=sum(Merged =="Uttar Pradesh")/length(state),
            q4_uttarakhand_nr=sum(Merged =="Uttarakhand")/length(state),
            q4_wb_nr=sum(Merged =="West Bengal")/length(state),.groups='drop') %>% 
  ungroup() %>% 
  mutate(total=rowSums(select_if(., is.numeric), na.rm = TRUE))

summary_nr_q4 <- summary_nr_q4 %>% 
  mutate(state_weight=ias_processed_dta$state_weight) %>% 
  mutate(sdg_weight=ias_processed_dta$sdg_weight)

#241 officers selected their own state 
#They are from the following states:
bias_q4<-which(non_repeated_q4$REPEAT==1)
biased_officers_q4<-as.data.frame(table(non_repeated_q4$X12[bias_q4])) %>% 
  dplyr::select(-Var1) %>% 
  rename(No.of_Bias=Freq)

cadre_strength_q4<-as.data.frame(table(full_serving$X12)) %>% 
  filter(!Var1=="Nagaland", !Var1=="Uttarakhand") %>% 
  rename(Cadre=Var1) %>% 
  rename(Cadre_Strength=Freq)
#Table displaying the proportion of officers who showed bias in each state
compare_q4<-cbind(cadre_strength_q4, biased_officers_q4) %>% 
  mutate("Proportion of Biased Responses (Q4)"=round(No.of_Bias/Cadre_Strength*100,2))%>% 
  rename("Number of Biased Responses (Q4)" = No.of_Bias) %>% 
  rename("Strength of Cadre in Sample" = Cadre_Strength)
```

```{r, include=FALSE}
#Calculating unweighted averages for question 4 (for cumulative and biased responses)
q4_state_answers1 <- full_serving %>% 
  dplyr::select (state, Merged1.1, X12) %>% 
  mutate (Merged = Merged1.1) %>% 
  dplyr:: select(-Merged1.1)
...
summary_q4_unweighted <- state_answers_q4 %>% 
  summarise("Andaman and Nicobar Islands"=round(sum(Merged =="Andaman and Nicobar Islands")*100/length(Merged),2),
            "Andhra Pradesh"=round(sum(Merged =="Andhra Pradesh")*100/length(Merged),2), 
            "Arunachal Pradesh"=round(sum(Merged =="Arunachal Pradesh")*100/length(Merged),2),
            Assam=round(sum(Merged =="Assam")*100/length(Merged),2),
            Bihar=round(sum(Merged =="Bihar")*100/length(Merged),2),
            Chandigarh= round(sum(Merged =="Chandigarh")*100/length(Merged),2),
            Chhattisgarh=round(sum(Merged =="Chhattisgarh")*100/length(Merged),2),
            DNHDD=round(sum(Merged =="Dadra and Nagar Haveli and Daman and Dui")*100/length(Merged),2),
            Goa=round(sum(Merged =="Goa")*100/length(Merged),2),
            Gujarat=round(sum(Merged =="Gujarat")*100/length(Merged),2),
            Haryana=round(sum(Merged =="Haryana")*100/length(Merged),2),
            "Himachal Pradesh"=round(sum(Merged =="Himachal Pradesh")*100/length(Merged),2),
            "Jammu and Kashmir"=round(sum(Merged =="Jammu and Kashmir")*100/length(Merged),2),
            Jharkhand=round(sum(Merged =="Jharkhand")*100/length(Merged),2),
            Karnataka=round(sum(Merged =="Karnataka")*100/length(Merged),2),
            Kerala=round(sum(Merged =="Kerala")*100/length(Merged),2),
            Ladakh=round(sum(Merged =="Ladakh")*100/length(Merged),2),
            "Lakshadweep Islands"=round(sum(Merged =="Lakshadweep")*100/length(Merged),2),
            "Madhya Pradesh"=round(sum(Merged =="Madhya Pradesh")*100/length(Merged),2),
            Maharashtra=round(sum(Merged =="Maharashtra")*100/length(Merged),2),
            Manipur=round(sum(Merged =="Manipur")*100/length(Merged),2),
            Meghalaya=round(sum(Merged =="Meghalaya")*100/length(Merged),2),
            Mizoram=round(sum(Merged =="Mizoram")*100/length(Merged),2),
            Nagaland=round(sum(Merged =="Nagaland")*100/length(Merged),2),
            Delhi=round(sum(Merged =="NCT of Delhi")*100/length(Merged),2),
            None=round(sum(Merged =="No state has performed well")*100/length(Merged),2),
            Odisha=round(sum(Merged =="Odisha")*100/length(Merged),2),
            Puducherry=round(sum(Merged =="Puducherry")*100/length(Merged),2),
            Punjab=round(sum(Merged =="Punjab")*100/length(Merged),2),
            Rajasthan=round(sum(Merged =="Rajasthan")*100/length(Merged),2),
            Sikkim=round(sum(Merged =="Sikkim")*100/length(Merged),2),
            "Tamil Nadu"=round(sum(Merged =="Tamil Nadu")*100/length(Merged),2),
            Telangana=round(sum(Merged =="Telangana")*100/length(Merged),2),
            Tripura=round(sum(Merged =="Tripura")*100/length(Merged),2),
            "Uttar Pradesh"=round(sum(Merged =="Uttar Pradesh")*100/length(Merged),2),
            Uttarakhand=round(sum(Merged =="Uttarakhand")*100/length(Merged),2),
            "West Bengal"=round(sum(Merged =="West Bengal")*100/length(Merged),2)) %>% 
  ungroup() 
...
#Calculating averages for *unbiased* officers only (by filtering out all officers who selected their own merged)
non_repeated_q4 <- state_answers_q4 %>% 
mutate (REPEAT = ifelse (Merged == "Andaman and Nicobar Islands " & X12 == "AGMUT", 1,
ifelse (Merged == "Andhra Pradesh" & X12 == "Andhra Pradesh", 1,
ifelse (Merged == "Arunachal Pradesh" & X12 == "AGMUT", 1, 
...
summary_nr_q4_unweighted <- nr_q4 %>% 
  na.omit() %>% 
 summarise("Andaman and Nicobar Islands"=round(sum(Merged =="Andaman and Nicobar Islands")*100/length(Merged),2),
            "Andhra Pradesh"=round(sum(Merged =="Andhra Pradesh")*100/length(Merged),2), 
            "Arunachal Pradesh"=round(sum(Merged =="Arunachal Pradesh")*100/length(Merged),2),
...
#Calculating averages for *unbiased* young officers only (by filtering out all officers who selected their own state)

state_answers_q4_young_nr <- state_answers_q4_young %>% 
mutate (REPEAT = ifelse (Merged == "Andaman and Nicobar Islands " & X12 == "AGMUT", 1,
ifelse (Merged == "Andhra Pradesh" & X12 == "Andhra Pradesh", 1,
ifelse (Merged == "Arunachal Pradesh" & X12 == "AGMUT", 1, 
ifelse (Merged == "Assam" & X12 == "Assam Meghalaya", 1,
...
nr_q4_young <- state_answers_q4_young_nr %>% 
filter (REPEAT == 0)

summary_nr_q4_young <- nr_q4_young %>% 
  group_by(state) %>% 
  na.omit() %>% 
  summarise(q4_anislands=sum(Merged =="Andaman and Nicobar Islands")/length(state),
            q4_andhra=sum(Merged =="Andhra Pradesh")/length(state), 
  ...
  add_row(state="Puducherry") %>% 
  add_row(state="Mizoram") %>% 
  add_row(state="Arunachal Pradesh") %>% 
  arrange(state) 
#There are no young officers from A and N Islands, Chandigarh, Lakshadweep Islands, Puducherry, Arunachal Pradesh and Mizoram. The summary produced for young officers, thus, is five rows shorter. In order to cbind this dataset with other (like ias_processed_dta) for weight calculations, we have added these states with NAs to match the number of rows across datasets. This is done for all summaries for young and old officers.
summary_nr_q4_young <- summary_nr_q4_young %>% 
mutate(state_young_weight=ias_processed_dta$state_young_weight)
```
...
#Calculating averages for *unbiased* old officers only (by filtering out all officers who selected their own state)

state_answers_q4_old_nr <- state_answers_q4_old %>% 
mutate (REPEAT = ifelse (Merged == "Andaman and Nicobar Islands " & X12 == "AGMUT", 1,
ifelse (Merged == "Andhra Pradesh" & X12 == "Andhra Pradesh", 1,
ifelse (Merged == "Arunachal Pradesh" & X12 == "AGMUT", 1, 
ifelse (Merged == "Assam" & X12 == "Assam Meghalaya", 1,
...
nr_q4_old <- state_answers_q4_old_nr %>% 
filter (REPEAT == 0)

summary_nr_q4_old <- nr_q4_old %>% 
  group_by(state) %>% 
  na.omit() %>% 
  summarise(q4_anislands=sum(Merged =="Andaman and Nicobar Islands")/length(state),
            q4_andhra=sum(Merged =="Andhra Pradesh")/length(state), 
  ...
  mutate(total=rowSums(select_if(., is.numeric), na.rm = TRUE)) %>% 
  add_row(state="Nagaland") %>% 
  add_row(state="Ladakh") %>%
  arrange(state) 
#There are no old officers from Nagaland. The summary produced for old officers, thus, is one rows shorter. In order to cbind this dataset with other (like ias_processed_dta) for weight calculations, we have added these states with NAs to match the number of rows across datasets. This is done for all summaries for young and old officers.
summary_nr_q4_old <- summary_nr_q4_old %>% 
  mutate(state_old_weight=ias_processed_dta$state_old_weight)
```
...
q4_state_answers1_former <- full_former %>% 
  dplyr::select (Merged1.1, X12) %>% 
  mutate (Merged = Merged1.1) %>% 
  dplyr:: select(-Merged1.1)

state_answers_q4_former <- full_former %>% 
  dplyr::select (Merged1.2, X12)%>% 
  mutate (Merged = Merged1.2) %>% 
  dplyr:: select (-Merged1.2) %>% 
...
#Calculating averages for *unbiased* former officers only (by filtering out all officers who selected their own state)

state_answers_q4_former_nr <- state_answers_q4_former %>% 
mutate (REPEAT = ifelse (Merged == "Andaman and Nicobar Islands " & X12 == "AGMUT", 1,
ifelse (Merged == "Andhra Pradesh" & X12 == "Andhra Pradesh", 1,
ifelse (Merged == "Arunachal Pradesh" & X12 == "AGMUT", 1, 
ifelse (Merged == "Assam" & X12 == "Assam Meghalaya", 1,
...
nr_q4_former <- state_answers_q4_former_nr %>% 
filter (REPEAT == 0)

summary_nr_q4_former <- nr_q4_former %>%  
  na.omit() %>% 
  summarise(q4_anislands=sum(Merged =="Andaman and Nicobar Islands")/length(Merged),
            q4_andhra=sum(Merged =="Andhra Pradesh")/length(Merged), 
            q4_arunachal=sum(Merged =="Arunachal Pradesh")/length(Merged),
...
table(state_answers_q5$X12[bias])

#filtering out all officers who selected their own state 
state_answers_q5_unbiased <- state_answers_q5 %>% 
  filter(!state_bias==1)
...
#Calculating averages for question 5 without state grouping
summary_q5_nonweighted<- state_answers_q5 %>% 
  na.omit() %>% 
  summarise(q5_anislands=sum(Merged=="Andaman and Nicobar Islands")/ length(Merged),
            ...
            q5_none=sum(Merged=="No state has performed poorly")/length(Merged),
            ...
            q5_wb=sum(Merged=="West Bengal")/length(Merged))
...
#They are from the following states:
bias_q6<-which(q6$state_bias==1)
biased_officers_q6<-as.data.frame(table(q6$X12[bias_q6])) %>% 
  dplyr::select(-Var1) %>% 
  rename(No.of_Bias=Freq)

cadre_strength_q6<-as.data.frame(table(full_serving$X12)) %>% 
  filter(!Var1=="Chattisgarh", !Var1=="Manipur", !Var1=="Nagaland", !Var1=="Uttarakhand") %>% 
  rename(Cadre=Var1) %>% 
  rename(Cadre_Strength=Freq)
#Table displaying the proportion of officers who showed bias in each state
compare_q6<-cbind(cadre_strength_q6, biased_officers_q6) %>% 
  mutate("Proportion of Biased Responses (Q6)"=round(No.of_Bias/Cadre_Strength*100,2)) %>% 
  rename("Number of Biased Responses (Q6)" = No.of_Bias) %>% 
  rename("Strength of Cadre in Sample" = Cadre_Strength) 
...
#Calculating averages for Q6 for all officers without state grouping
summary_q6_nonweighted <- q6 %>% 
  na.omit() %>% 
  summarise(q6_anislands=sum(Merged =="Andaman and Nicobar Islands")/length(Merged),
            ...
            q6_wb=sum(Merged =="West Bengal")/length(Merged)) 

#Calculating averages for Q6 for *unbiased* officers only
...
#filtering out all officers who selected their own state 
state_answers_q6_young_unbiased <- state_answers_q6_young %>% 
  filter(!state_bias==1)
...
summary_q6_old_unbiased <- state_answers_q6_old_unbiased %>% 
  ...
  add_row(state="Nagaland") %>%
  add_row(state="Andaman and Nicobar Islands") %>%
  add_row(state="DNH and DD") %>%
  add_row(state="Jammu & Kashmir") %>%
  arrange(state) 
#There are no old officers from Nagaland. The summary produced for young officers, thus, is one row shorter. In order to cbind this dataset with other (like ias_processed_dta) for weight calculations, we have added these states with NAs to match the number of rows across datasets. This is done for all summaries for young and old officers.
...
#filtering out all officers who selected their own state 
state_answers_q6_former_unbiased <- state_answers_q6_former %>% 
  filter(!state_bias==1)
...
```{r}
#Creating a table with responses to Q 4, 5 and 6 for all officers and for unbiased officers 
summary_q4_unweighted<-as.data.frame(t(summary_q4_unweighted)) %>% 
  rename("q4_prop"=V1)
summary_nr_q4_unweighted<-as.data.frame(t(summary_nr_q4_unweighted)) %>% 
  rename("q4_unbias_prop"=V1)
summary_q5_nonweighted<-as.data.frame(t(summary_q5_nonweighted)) %>% 
  mutate(V1=round(V1*100, digits=2)) %>%
  rename("q5_prop"=V1)
summary_q5_unbiased<-as.data.frame(t(summary_q5_unbiased)) %>%
  mutate(V1=round(V1*100, digits=2)) %>% 
  rename("q5_unbias_prop"=V1)
summary_q6_nonweighted<-as.data.frame(t(summary_q6_nonweighted)) %>% 
  mutate(V1=round(V1*100, digits=2)) %>%
  rename("q6_prop"=V1)
summary_q6_unbiased<-as.data.frame(t(summary_q6_unbiased)) %>% 
  mutate(V1=round(V1*100, digits=2)) %>%
  rename("q6_unbias_prop"=V1)

state_responses<-cbind(summary_q4_unweighted, summary_nr_q4_unweighted, summary_q5_nonweighted, summary_q5_unbiased, summary_q6_nonweighted, summary_q6_unbiased) 

state_responses<-setNames(cbind(rownames(state_responses), state_responses, row.names = NULL), 
         c("Responses (Selected States)", "Q4 Proportion of All Responses (%)", "Q4 Responses Excluding those where Officers Selected their Own State (%)", "Q5 Proportion of All Responses (%)", "Q5 Responses Excluding those where Officers Selected their Own State (%)",  "Q6 Proportion of All Responses (%)", "Q6 Responses Excluding those where Officers Selected their Own State (%)"))

state_responses %>%
  kbl(caption = "Officer responses for Q4, Q5 and Q6")%>%
  kable_classic(full_width = F, html_font = "Cambria", font_size = 13) %>% 
  footnote(number = c("Question 4: 241 officers (out of 452) selected their own state", "Question 5: 42 officers (out of 452) selected their own state" ,"Question 6: 117 officers (out of 452) selected their own state"))
...
## Question 4: Aggregate Graph: Accounting for Officers' Bias

Please select upto two States that you think have best fulfilled their pandemic management responsibilities. 

241 out of 452 officers felt that their own state has been best at carrying out their pandemic management responsibilities. These officers may be biased in their response. The following graph excludes all such potentially biased responses for question 4.

\vspace{6mm}
```{r echo=FALSE, fig.height=10, fig.width=18, warning=FALSE}
#Aggregate
q4full_mean_anislands_nr <- round(weighted.mean(summary_nr_q4$q4_anislands_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_andhra_nr <- round(weighted.mean(summary_nr_q4$q4_andhra_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_arunachal_nr <- round(weighted.mean(summary_nr_q4$q4_arunachal_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_assam_nr <- round(weighted.mean(summary_nr_q4$q4_assam_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_bihar_nr <- round(weighted.mean(summary_nr_q4$q4_bihar_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_chandigarh_nr <- round(weighted.mean(summary_nr_q4$q4_chandigarh_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_chhattisgarh_nr <- round(weighted.mean(summary_nr_q4$q4_chhattisgarh_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_dnhdd_nr <- round(weighted.mean(summary_nr_q4$q4_dnhdd_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_goa_nr <- round(weighted.mean(summary_nr_q4$q4_goa_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_gujarat_nr <- round(weighted.mean(summary_nr_q4$q4_gujarat_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_haryana_nr <- round(weighted.mean(summary_nr_q4$q4_haryana_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_hp_nr <- round(weighted.mean(summary_nr_q4$q4_hp_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_jk_nr <- round(weighted.mean(summary_nr_q4$q4_jk_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_jharkhand_nr <- round(weighted.mean(summary_nr_q4$q4_jharkhand_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_karnataka_nr <- round(weighted.mean(summary_nr_q4$q4_karnataka_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_kerala_nr <- round(weighted.mean(summary_nr_q4$q4_kerala_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_ladakh_nr <- round(weighted.mean(summary_nr_q4$q4_ladakh_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_lakshadweep_nr <- round(weighted.mean(summary_nr_q4$q4_lakshadweep_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_mp_nr <- round(weighted.mean(summary_nr_q4$q4_mp_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_maharashtra_nr <- round(weighted.mean(summary_nr_q4$q4_maharashtra_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_manipur_nr <- round(weighted.mean(summary_nr_q4$q4_manipur_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_meghalaya_nr <- round(weighted.mean(summary_nr_q4$q4_meghalaya_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_mizoram_nr <- round(weighted.mean(summary_nr_q4$q4_mizoram_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_nagaland_nr <- round(weighted.mean(summary_nr_q4$q4_nagaland_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_delhi_nr <- round(weighted.mean(summary_nr_q4$q4_delhi_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_none_nr <- round(weighted.mean(summary_nr_q4$q4_none_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_odisha_nr <- round(weighted.mean(summary_nr_q4$q4_odisha_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_puducherry_nr <- round(weighted.mean(summary_nr_q4$q4_puducherry_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_punjab_nr <- round(weighted.mean(summary_nr_q4$q4_punjab_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_rajasthan_nr <- round(weighted.mean(summary_nr_q4$q4_rajasthan_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_sikkim_nr <- round(weighted.mean(summary_nr_q4$q4_sikkim_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_tn_nr <- round(weighted.mean(summary_nr_q4$q4_tn_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_telangana_nr <- round(weighted.mean(summary_nr_q4$q4_telangana_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_tripura_nr <- round(weighted.mean(summary_nr_q4$q4_tripura_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_up_nr <- round(weighted.mean(summary_nr_q4$q4_up_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_uttarakhand_nr <- round(weighted.mean(summary_nr_q4$q4_uttarakhand_nr, summary_nr_q4$state_weight)*100,2)
q4full_mean_wb_nr <- round(weighted.mean(summary_nr_q4$q4_wb_nr, summary_nr_q4$state_weight)*100,2)

q4full_nr_plot <- c(q4full_mean_anislands_nr, q4full_mean_andhra_nr, q4full_mean_arunachal_nr, q4full_mean_assam_nr, q4full_mean_bihar_nr, q4full_mean_chandigarh_nr, q4full_mean_chhattisgarh_nr, q4full_mean_delhi_nr, q4full_mean_dnhdd_nr, q4full_mean_goa_nr, q4full_mean_gujarat_nr, q4full_mean_haryana_nr, q4full_mean_hp_nr, q4full_mean_jharkhand_nr, q4full_mean_jk_nr, q4full_mean_karnataka_nr, q4full_mean_kerala_nr, q4full_mean_ladakh_nr, q4full_mean_lakshadweep_nr, q4full_mean_maharashtra_nr, q4full_mean_manipur_nr, q4full_mean_meghalaya_nr, q4full_mean_mizoram_nr, q4full_mean_mp_nr, q4full_mean_nagaland_nr, q4full_mean_none_nr, q4full_mean_odisha_nr, q4full_mean_puducherry_nr, q4full_mean_punjab_nr, q4full_mean_rajasthan_nr, q4full_mean_sikkim_nr, q4full_mean_telangana_nr, q4full_mean_tn_nr, q4full_mean_tripura_nr, q4full_mean_up_nr, q4full_mean_uttarakhand_nr, q4full_mean_wb_nr)


q4full_plot_labels <- c("AN", "AP", "AR", "AS", "BR", "CH", "CG", "DL", "DNH & DD", "GA", "GJ", "HR", "HP", "JH", "JK", "KA", "KL", "LA", "LD", "MH", "MN", "ML", "MZ", "MP", "NL", "None", "OR", "PY", "PB", "RJ", "SK", "TG", "TN", "TR", "UP", "UK", "WB")

q4full_plot_nr_dta <- data.frame (q4full_nr_plot, q4full_plot_labels) 
q4full_plot_nr_dta$wrapped_labels = str_wrap(q4full_plot_nr_dta$q4full_plot_labels, width = 10)
q4full_nr_plot <- ggplot(q4full_plot_nr_dta, aes(x=q4full_plot_labels, y=q4full_nr_plot, fill = q4full_plot_labels)) +
  geom_bar(stat="identity", width= 1, show.legend = FALSE) + 
  geom_text(aes(label=paste0(sprintf("%0.2f", q4full_nr_plot),"%"), vjust=-0.3), size=3.5) +
  scale_y_continuous(limits = c(0,40)) +
  scale_x_discrete(labels = function(q4full_plot_labels) str_wrap(q4full_plot_labels, width = 30)) +
  labs(x="",y="Percent")+
  newtheme
...
## Question 4: Disaggregated by Age: Accounting for Officers' Bias

Please select upto two States that you think have best fulfilled their pandemic management responsibilities. (States selected in previous answers can be repeated)  

132 out of the 248 young officers (who joined the IAS in 2012 or later) felt that their own state has been under-appreciated for its successful pandemic management. 110 of the 204 older officers and 38 of 96 former officers were similarly biased in their responses to this question. The following graphs exclude these potentialy biased responses. 

\vspace{6mm}
```{r, echo = FALSE, fig.height=17,fig.width=18, warning=FALSE}
#Dis-aggregated by Age (by omitting responses identical to the cadre)

q4young_mean_anislands_nr <- round(weighted.mean(summary_nr_q4_young$q4_anislands, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_andhra_nr <- round(weighted.mean(summary_nr_q4_young$q4_andhra, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_arunachal_nr <- round(weighted.mean(summary_nr_q4_young$q4_arunachal, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_assam_nr <- round(weighted.mean(summary_nr_q4_young$q4_assam, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_bihar_nr <- round(weighted.mean(summary_nr_q4_young$q4_bihar, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_chandigarh_nr <- round(weighted.mean(summary_nr_q4_young$q4_chandigarh, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_chhattisgarh_nr <- round(weighted.mean(summary_nr_q4_young$q4_chhattisgarh, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_dnhdd_nr <- round(weighted.mean(summary_nr_q4_young$q4_dnhdd, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_goa_nr <- round(weighted.mean(summary_nr_q4_young$q4_goa, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_gujarat_nr <- round(weighted.mean(summary_nr_q4_young$q4_gujarat, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_haryana_nr <- round(weighted.mean(summary_nr_q4_young$q4_haryana, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_hp_nr <- round(weighted.mean(summary_nr_q4_young$q4_hp, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_jk_nr <- round(weighted.mean(summary_nr_q4_young$q4_jk, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_jharkhand_nr <- round(weighted.mean(summary_nr_q4_young$q4_jharkhand, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_karnataka_nr <- round(weighted.mean(summary_nr_q4_young$q4_karnataka, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_kerala_nr <- round(weighted.mean(summary_nr_q4_young$q4_kerala, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_ladakh_nr <- round(weighted.mean(summary_nr_q4_young$q4_ladakh, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_lakshadweep_nr <- round(weighted.mean(summary_nr_q4_young$q4_lakshadweep, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_mp_nr <- round(weighted.mean(summary_nr_q4_young$q4_mp, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_maharashtra_nr <- round(weighted.mean(summary_nr_q4_young$q4_maharashtra, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_manipur_nr <- round(weighted.mean(summary_nr_q4_young$q4_manipur, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_meghalaya_nr <- round(weighted.mean(summary_nr_q4_young$q4_meghalaya, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_mizoram_nr <- round(weighted.mean(summary_nr_q4_young$q4_mizoram, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_nagaland_nr <- round(weighted.mean(summary_nr_q4_young$q4_nagaland, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_delhi_nr <- round(weighted.mean(summary_nr_q4_young$q4_delhi, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_none_nr <- round(weighted.mean(summary_nr_q4_young$q4_none, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_odisha_nr <- round(weighted.mean(summary_nr_q4_young$q4_odisha, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_puducherry_nr <- round(weighted.mean(summary_nr_q4_young$q4_puducherry, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_punjab_nr <- round(weighted.mean(summary_nr_q4_young$q4_punjab, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_rajasthan_nr <- round(weighted.mean(summary_nr_q4_young$q4_rajasthan, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_sikkim_nr <- round(weighted.mean(summary_nr_q4_young$q4_sikkim, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_tn_nr <- round(weighted.mean(summary_nr_q4_young$q4_tn, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_telangana_nr <- round(weighted.mean(summary_nr_q4_young$q4_telangana, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_tripura_nr <- round(weighted.mean(summary_nr_q4_young$q4_tripura, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_up_nr <- round(weighted.mean(summary_nr_q4_young$q4_up, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_uttarakhand_nr <- round(weighted.mean(summary_nr_q4_young$q4_uttarakhand, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)
q4young_mean_wb_nr <- round(weighted.mean(summary_nr_q4_young$q4_wb, summary_nr_q4_young$state_young_weight,na.rm=TRUE)*100,2)

q4young_nr_plot <- c(q4young_mean_anislands_nr, q4young_mean_andhra_nr, q4young_mean_arunachal_nr, q4young_mean_assam_nr, q4young_mean_bihar_nr, q4young_mean_chandigarh_nr, q4young_mean_chhattisgarh_nr, q4young_mean_delhi_nr, q4young_mean_dnhdd_nr, q4young_mean_goa_nr, q4young_mean_gujarat_nr, q4young_mean_haryana_nr, q4young_mean_hp_nr, q4young_mean_jharkhand_nr, q4young_mean_jk_nr, q4young_mean_karnataka_nr, q4young_mean_kerala_nr, q4young_mean_ladakh_nr, q4young_mean_lakshadweep_nr, q4young_mean_maharashtra_nr, q4young_mean_manipur_nr, q4young_mean_meghalaya_nr, q4young_mean_mizoram_nr, q4young_mean_mp_nr, q4young_mean_nagaland_nr, q4young_mean_none_nr, q4young_mean_odisha_nr, q4young_mean_puducherry_nr, q4young_mean_punjab_nr, q4young_mean_rajasthan_nr, q4young_mean_sikkim_nr, q4young_mean_telangana_nr, q4young_mean_tn_nr, q4young_mean_tripura_nr, q4young_mean_up_nr, q4young_mean_uttarakhand_nr, q4young_mean_wb_nr)

q4young_plot_labels <- c("AN", "AP", "AR", "AS", "BR", "CH", "CG", "DL", "DNH & DD", "GA", "GJ", "HR", "HP", "JH", "JK", "KA", "KL", "LA", "LD", "MH", "MN", "ML", "MZ", "MP", "NL", "None", "OR", "PY", "PB", "RJ", "SK", "TG", "TN", "TR", "UP", "UK", "WB")

q4young_nr_plot_dta <- data.frame (q4young_nr_plot, q4young_plot_labels) 
q4young_nr_plot_dta$wrapped_labels = str_wrap(q4young_nr_plot_dta$q4young_plot_labels, width = 10)
q4young_nr_plot <- ggplot(q4young_nr_plot_dta, aes(x=q4young_plot_labels, y=q4young_nr_plot, fill = q4young_plot_labels)) +
  geom_bar(stat="identity", width= 1, show.legend = FALSE) + 
  geom_text(aes(label=paste0(sprintf("%0.2f", q4young_nr_plot),"%"), vjust=-0.3), size=3.5) +
  scale_y_continuous(limits = c(0,40)) +
  scale_x_discrete(labels = function(q4young_plot_labels) str_wrap(q4young_plot_labels, width = 30)) +
  labs(x="",y="Percent", title = "2012 and After")+
  newtheme

q4old_mean_anislands_nr <- round(weighted.mean(summary_nr_q4_old$q4_anislands, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_andhra_nr <- round(weighted.mean(summary_nr_q4_old$q4_andhra, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_arunachal_nr <- round(weighted.mean(summary_nr_q4_old$q4_arunachal, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_assam_nr <- round(weighted.mean(summary_nr_q4_old$q4_assam, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_bihar_nr <- round(weighted.mean(summary_nr_q4_old$q4_bihar, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_chandigarh_nr <- round(weighted.mean(summary_nr_q4_old$q4_chandigarh, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_chhattisgarh_nr <- round(weighted.mean(summary_nr_q4_old$q4_chhattisgarh, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_dnhdd_nr <- round(weighted.mean(summary_nr_q4_old$q4_dnhdd, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_goa_nr <- round(weighted.mean(summary_nr_q4_old$q4_goa, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_gujarat_nr <- round(weighted.mean(summary_nr_q4_old$q4_gujarat, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_haryana_nr <- round(weighted.mean(summary_nr_q4_old$q4_haryana, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_hp_nr <- round(weighted.mean(summary_nr_q4_old$q4_hp, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_jk_nr <- round(weighted.mean(summary_nr_q4_old$q4_jk, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_jharkhand_nr <- round(weighted.mean(summary_nr_q4_old$q4_jharkhand, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_karnataka_nr <- round(weighted.mean(summary_nr_q4_old$q4_karnataka, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_kerala_nr <- round(weighted.mean(summary_nr_q4_old$q4_kerala, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_ladakh_nr <- round(weighted.mean(summary_nr_q4_old$q4_ladakh, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_lakshadweep_nr <- round(weighted.mean(summary_nr_q4_old$q4_lakshadweep, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_mp_nr <- round(weighted.mean(summary_nr_q4_old$q4_mp, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_maharashtra_nr <- round(weighted.mean(summary_nr_q4_old$q4_maharashtra, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_manipur_nr <- round(weighted.mean(summary_nr_q4_old$q4_manipur, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_meghalaya_nr <- round(weighted.mean(summary_nr_q4_old$q4_meghalaya, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_mizoram_nr <- round(weighted.mean(summary_nr_q4_old$q4_mizoram, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_nagaland_nr <- round(weighted.mean(summary_nr_q4_old$q4_nagaland, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_delhi_nr <- round(weighted.mean(summary_nr_q4_old$q4_delhi, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_none_nr <- round(weighted.mean(summary_nr_q4_old$q4_none, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_odisha_nr <- round(weighted.mean(summary_nr_q4_old$q4_odisha, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_puducherry_nr <- round(weighted.mean(summary_nr_q4_old$q4_puducherry, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_punjab_nr <- round(weighted.mean(summary_nr_q4_old$q4_punjab, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_rajasthan_nr <- round(weighted.mean(summary_nr_q4_old$q4_rajasthan, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_sikkim_nr <- round(weighted.mean(summary_nr_q4_old$q4_sikkim, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_tn_nr <- round(weighted.mean(summary_nr_q4_old$q4_tn, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_telangana_nr <- round(weighted.mean(summary_nr_q4_old$q4_telangana, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_tripura_nr <- round(weighted.mean(summary_nr_q4_old$q4_tripura, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_up_nr <- round(weighted.mean(summary_nr_q4_old$q4_up, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_uttarakhand_nr <- round(weighted.mean(summary_nr_q4_old$q4_uttarakhand, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)
q4old_mean_wb_nr <- round(weighted.mean(summary_nr_q4_old$q4_wb, summary_nr_q4_old$state_old_weight, na.rm = TRUE)*100,2)

q4old_nr_plot <- c(q4old_mean_anislands_nr, q4old_mean_andhra_nr, q4old_mean_arunachal_nr, q4old_mean_assam_nr, q4old_mean_bihar_nr, q4old_mean_chandigarh_nr, q4old_mean_chhattisgarh_nr, q4old_mean_delhi_nr, q4old_mean_dnhdd_nr, q4old_mean_goa_nr, q4old_mean_gujarat_nr, q4old_mean_haryana_nr, q4old_mean_hp_nr, q4old_mean_jharkhand_nr, q4old_mean_jk_nr, q4old_mean_karnataka_nr, q4old_mean_kerala_nr, q4old_mean_ladakh_nr, q4old_mean_lakshadweep_nr, q4old_mean_maharashtra_nr, q4old_mean_manipur_nr, q4old_mean_meghalaya_nr, q4old_mean_mizoram_nr, q4old_mean_mp_nr, q4old_mean_nagaland_nr, q4old_mean_none_nr, q4old_mean_odisha_nr, q4old_mean_puducherry_nr, q4old_mean_punjab_nr, q4old_mean_rajasthan_nr, q4old_mean_sikkim_nr, q4old_mean_telangana_nr, q4old_mean_tn_nr, q4old_mean_tripura_nr, q4old_mean_up_nr, q4old_mean_uttarakhand_nr, q4old_mean_wb_nr)

q4old_plot_labels <- c("AN", "AP", "AR", "AS", "BR", "CH", "CG", "DL", "DNH & DD", "GA", "GJ", "HR", "HP", "JH", "JK", "KA", "KL", "LA", "LD", "MH", "MN", "ML", "MZ", "MP", "NL", "None", "OR", "PY", "PB", "RJ", "SK", "TG", "TN", "TR", "UP", "UK", "WB")

q4old_nr_plot_dta <- data.frame (q4old_nr_plot, q4old_plot_labels) 
q4old_nr_plot_dta$wrapped_labels = str_wrap(q4old_nr_plot_dta$q4old_plot_labels, width = 10)
q4old_nr_plot <- ggplot(q4old_nr_plot_dta, aes(x=q4old_plot_labels, y=q4old_nr_plot, fill = q4old_plot_labels)) +
  geom_bar(stat="identity", width= 1, show.legend = FALSE) + 
  geom_text(aes(label=paste0(sprintf("%0.2f", q4old_nr_plot),"%"), vjust=-0.3), size=3.5) +
  scale_y_continuous(limits = c(0,40)) +
  scale_x_discrete(labels = function(q4old_plot_labels) str_wrap(q4old_plot_labels, width = 30)) +
  labs(x="",y="Percent", title = "2011 and Before")+
  newtheme

q4former_mean_anislands_nr <- summary_nr_q4_former$q4_anislands*100
q4former_mean_andhra_nr <- summary_nr_q4_former$q4_andhra*100
q4former_mean_arunachal_nr <- summary_nr_q4_former$q4_arunachal*100
q4former_mean_assam_nr <- summary_nr_q4_former$q4_assam*100
q4former_mean_bihar_nr <- summary_nr_q4_former$q4_bihar*100
q4former_mean_chandigarh_nr <- summary_nr_q4_former$q4_chandigarh*100
q4former_mean_chhattisgarh_nr <- summary_nr_q4_former$q4_chhattisgarh*100
q4former_mean_delhi_nr <- summary_nr_q4_former$q4_delhi*100
q4former_mean_dnhdd_nr <- summary_nr_q4_former$q4_dnhdd*100
q4former_mean_goa_nr <- summary_nr_q4_former$q4_goa*100
q4former_mean_gujarat_nr <- summary_nr_q4_former$q4_gujarat*100
q4former_mean_haryana_nr <- summary_nr_q4_former$q4_haryana*100
q4former_mean_hp_nr <- summary_nr_q4_former$q4_hp*100
q4former_mean_jharkhand_nr <- summary_nr_q4_former$q4_jharkhand*100
q4former_mean_jk_nr <- summary_nr_q4_former$q4_jk*100
q4former_mean_karnataka_nr <- summary_nr_q4_former$q4_karnataka*100
q4former_mean_kerala_nr <- summary_nr_q4_former$q4_kerala*100
q4former_mean_lakshadweep_nr <- summary_nr_q4_former$q4_lakshadweep*100
q4former_mean_ladakh_nr <- summary_nr_q4_former$q4_ladakh*100
q4former_mean_maharashtra_nr <- summary_nr_q4_former$q4_maharashtra*100
q4former_mean_manipur_nr <- summary_nr_q4_former$q4_manipur*100
q4former_mean_meghalaya_nr <- summary_nr_q4_former$q4_meghalaya*100
q4former_mean_mizoram_nr <- summary_nr_q4_former$q4_mizoram*100
q4former_mean_mp_nr <- summary_nr_q4_former$q4_mp*100
q4former_mean_nagaland_nr <- summary_nr_q4_former$q4_nagaland*100
q4former_mean_none_nr <- summary_nr_q4_former$q4_none*100
q4former_mean_odisha_nr <- summary_nr_q4_former$q4_odisha*100
q4former_mean_puducherry_nr <- summary_nr_q4_former$q4_puducherry*100
q4former_mean_punjab_nr <-summary_nr_q4_former$q4_punjab*100
q4former_mean_rajasthan_nr <- summary_nr_q4_former$q4_rajasthan*100
q4former_mean_sikkim_nr <- summary_nr_q4_former$q4_sikkim*100
q4former_mean_telangana_nr <- summary_nr_q4_former$q4_telangana*100
q4former_mean_tn_nr <- summary_nr_q4_former$q4_tn*100
q4former_mean_tripura_nr <- summary_nr_q4_former$q4_tripura*100
q4former_mean_up_nr <- summary_nr_q4_former$q4_up*100
q4former_mean_uttarakhand_nr <- summary_nr_q4_former$q4_uttarakhand*100
q4former_mean_wb_nr <- summary_nr_q4_former$q4_wb*100

q4former_nr_plot <- c(q4former_mean_anislands_nr, q4former_mean_andhra_nr, q4former_mean_arunachal_nr, q4former_mean_assam_nr, q4former_mean_bihar_nr, q4former_mean_chandigarh_nr, q4former_mean_chhattisgarh_nr, q4former_mean_delhi_nr, q4former_mean_dnhdd_nr, q4former_mean_goa_nr, q4former_mean_gujarat_nr, q4former_mean_haryana_nr, q4former_mean_hp_nr, q4former_mean_jharkhand_nr, q4former_mean_jk_nr, q4former_mean_karnataka_nr, q4former_mean_kerala_nr, q4former_mean_ladakh_nr, q4former_mean_lakshadweep_nr, q4former_mean_maharashtra_nr, q4former_mean_manipur_nr, q4former_mean_meghalaya_nr, q4former_mean_mizoram_nr, q4former_mean_mp_nr, q4former_mean_nagaland_nr, q4former_mean_none_nr, q4former_mean_odisha_nr, q4former_mean_puducherry_nr, q4former_mean_punjab_nr, q4former_mean_rajasthan_nr, q4former_mean_sikkim_nr, q4former_mean_telangana_nr, q4former_mean_tn_nr, q4former_mean_tripura_nr, q4former_mean_up_nr, q4former_mean_uttarakhand_nr, q4former_mean_wb_nr)

q4former_plot_labels <- c("AN", "AP", "AR", "AS", "BR", "CH", "CG", "DL", "DNH & DD", "GA", "GJ", "HR", "HP", "JH", "JK", "KA", "KL", "LA", "LD", "MH", "MN", "ML", "MZ", "MP", "NL", "None", "OR", "PY", "PB", "RJ", "SK", "TG", "TN", "TR", "UP", "UK", "WB")

q4former_nr_plot_dta <- data.frame (q4former_nr_plot, q4former_plot_labels) 
q4former_nr_plot_dta$wrapped_labels = str_wrap(q4former_nr_plot_dta$q4former_plot_labels, width = 10)
q4former_nr_plot <- ggplot(q4former_nr_plot_dta, aes(x=q4former_plot_labels, y=q4former_nr_plot, fill = q4former_plot_labels)) +
  geom_bar(stat="identity", width= 1, show.legend = FALSE) + 
  geom_text(aes(label=paste0(sprintf("%0.2f", q4former_nr_plot),"%"), vjust=-0.3), size=3.5) +
  scale_y_continuous(limits = c(0,40)) +
  scale_x_discrete(labels = function(q4former_plot_labels) str_wrap(q4former_plot_labels, width = 30)) +
  labs(x="",y="Percent", title = "Former Officers")+
  newtheme

plot4_nr_fullage <- plot_grid(q4young_nr_plot,q4old_nr_plot,q4former_nr_plot, nrow=3)
plot4_nr_fullage
```
\newpage
...
## Question 4: Disaggregated by SDG: Accounting for Officers' Bias

Please select upto two States that you think have best fulfilled their pandemic management responsibilities. (States selected in previous answers can be repeated)  

Responses by officers who have selected their own states have been excluded from the following graph.
...
##Aspirants
q4aspt_weighted_mean_anislands_nr <- round(weighted.mean(summary_nr_q4$q4_anislands_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_andhra_nr <- round(weighted.mean(summary_nr_q4$q4_andhra_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_arunachal_nr <- round(weighted.mean(summary_nr_q4$q4_arunachal_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_assam_nr <- round(weighted.mean(summary_nr_q4$q4_assam_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_bihar_nr <- round(weighted.mean(summary_nr_q4$q4_bihar_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_chandigarh_nr <- round(weighted.mean(summary_nr_q4$q4_chandigarh_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_chhattisgarh_nr <- round(weighted.mean(summary_nr_q4$q4_chhattisgarh_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_dnhdd_nr <- round(weighted.mean(summary_nr_q4$q4_dnhdd_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_goa_nr <- round(weighted.mean(summary_nr_q4$q4_goa_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_gujarat_nr <- round(weighted.mean(summary_nr_q4$q4_gujarat_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_haryana_nr <- round(weighted.mean(summary_nr_q4$q4_haryana_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_hp_nr <- round(weighted.mean(summary_nr_q4$q4_hp_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_jk_nr <- round(weighted.mean(summary_nr_q4$q4_jk_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_jharkhand_nr <- round(weighted.mean(summary_nr_q4$q4_jharkhand_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_karnataka_nr <- round(weighted.mean(summary_nr_q4$q4_karnataka_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_kerala_nr <- round(weighted.mean(summary_nr_q4$q4_kerala_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_ladakh_nr <- round(weighted.mean(summary_nr_q4$q4_ladakh_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_lakshadweep_nr <- round(weighted.mean(summary_nr_q4$q4_lakshadweep_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_mp_nr <- round(weighted.mean(summary_nr_q4$q4_mp_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_maharashtra_nr <- round(weighted.mean(summary_nr_q4$q4_maharashtra_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_manipur_nr <- round(weighted.mean(summary_nr_q4$q4_manipur_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_meghalaya_nr <- round(weighted.mean(summary_nr_q4$q4_meghalaya_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_mizoram_nr <- round(weighted.mean(summary_nr_q4$q4_mizoram_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_nagaland_nr <- round(weighted.mean(summary_nr_q4$q4_nagaland_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_delhi_nr <- round(weighted.mean(summary_nr_q4$q4_delhi_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_none_nr <- round(weighted.mean(summary_nr_q4$q4_none_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_odisha_nr <- round(weighted.mean(summary_nr_q4$q4_odisha_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_puducherry_nr <- round(weighted.mean(summary_nr_q4$q4_puducherry_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_punjab_nr <- round(weighted.mean(summary_nr_q4$q4_punjab_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_rajasthan_nr <- round(weighted.mean(summary_nr_q4$q4_rajasthan_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_sikkim_nr <- round(weighted.mean(summary_nr_q4$q4_sikkim_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_tn_nr <- round(weighted.mean(summary_nr_q4$q4_tn_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_telangana_nr <- round(weighted.mean(summary_nr_q4$q4_telangana_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_tripura_nr <- round(weighted.mean(summary_nr_q4$q4_tripura_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_up_nr <- round(weighted.mean(summary_nr_q4$q4_up_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_uttarakhand_nr <- round(weighted.mean(summary_nr_q4$q4_uttarakhand_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)
q4aspt_weighted_mean_wb_nr <- round(weighted.mean(summary_nr_q4$q4_wb_nr[summary_nr_q4$sdg_tier=="Aspirant"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Aspirant"])*100,2)

##Performers
q4perf_weighted_mean_anislands_nr <- round(weighted.mean(summary_nr_q4$q4_anislands_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_andhra_nr <- round(weighted.mean(summary_nr_q4$q4_andhra_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_arunachal_nr <- round(weighted.mean(summary_nr_q4$q4_arunachal_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_assam_nr <- round(weighted.mean(summary_nr_q4$q4_assam_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_bihar_nr <- round(weighted.mean(summary_nr_q4$q4_bihar_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_chandigarh_nr <- round(weighted.mean(summary_nr_q4$q4_chandigarh_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_chhattisgarh_nr <- round(weighted.mean(summary_nr_q4$q4_chhattisgarh_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_dnhdd_nr <- round(weighted.mean(summary_nr_q4$q4_dnhdd_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_goa_nr <- round(weighted.mean(summary_nr_q4$q4_goa_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_gujarat_nr <- round(weighted.mean(summary_nr_q4$q4_gujarat_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_haryana_nr <- round(weighted.mean(summary_nr_q4$q4_haryana_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_hp_nr <- round(weighted.mean(summary_nr_q4$q4_hp_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_jk_nr <- round(weighted.mean(summary_nr_q4$q4_jk_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_jharkhand_nr <- round(weighted.mean(summary_nr_q4$q4_jharkhand_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_karnataka_nr <- round(weighted.mean(summary_nr_q4$q4_karnataka_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_kerala_nr <- round(weighted.mean(summary_nr_q4$q4_kerala_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_ladakh_nr <- round(weighted.mean(summary_nr_q4$q4_ladakh_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_lakshadweep_nr <- round(weighted.mean(summary_nr_q4$q4_lakshadweep_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_mp_nr <- round(weighted.mean(summary_nr_q4$q4_mp_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_maharashtra_nr <- round(weighted.mean(summary_nr_q4$q4_maharashtra_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_manipur_nr <- round(weighted.mean(summary_nr_q4$q4_manipur_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_meghalaya_nr <- round(weighted.mean(summary_nr_q4$q4_meghalaya_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_mizoram_nr <- round(weighted.mean(summary_nr_q4$q4_mizoram_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_nagaland_nr <- round(weighted.mean(summary_nr_q4$q4_nagaland_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_delhi_nr <- round(weighted.mean(summary_nr_q4$q4_delhi_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_none_nr <- round(weighted.mean(summary_nr_q4$q4_none_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_odisha_nr <- round(weighted.mean(summary_nr_q4$q4_odisha_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_puducherry_nr <- round(weighted.mean(summary_nr_q4$q4_puducherry_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_punjab_nr <- round(weighted.mean(summary_nr_q4$q4_punjab_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_rajasthan_nr <- round(weighted.mean(summary_nr_q4$q4_rajasthan_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_sikkim_nr <- round(weighted.mean(summary_nr_q4$q4_sikkim_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_tn_nr <- round(weighted.mean(summary_nr_q4$q4_tn_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_telangana_nr <- round(weighted.mean(summary_nr_q4$q4_telangana_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_tripura_nr <- round(weighted.mean(summary_nr_q4$q4_tripura_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_up_nr <- round(weighted.mean(summary_nr_q4$q4_up_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_uttarakhand_nr <- round(weighted.mean(summary_nr_q4$q4_uttarakhand_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)
q4perf_weighted_mean_wb_nr <- round(weighted.mean(summary_nr_q4$q4_wb_nr[summary_nr_q4$sdg_tier=="Performer"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Performer"])*100,2)

##Frontrunner
q4frnt_weighted_mean_anislands_nr <- round(weighted.mean(summary_nr_q4$q4_anislands_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_andhra_nr <- round(weighted.mean(summary_nr_q4$q4_andhra_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_arunachal_nr <- round(weighted.mean(summary_nr_q4$q4_arunachal_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_assam_nr <- round(weighted.mean(summary_nr_q4$q4_assam_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_bihar_nr <- round(weighted.mean(summary_nr_q4$q4_bihar_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_chandigarh_nr <- round(weighted.mean(summary_nr_q4$q4_chandigarh_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_chhattisgarh_nr <- round(weighted.mean(summary_nr_q4$q4_chhattisgarh_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_dnhdd_nr <- round(weighted.mean(summary_nr_q4$q4_dnhdd_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_goa_nr <- round(weighted.mean(summary_nr_q4$q4_goa_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_gujarat_nr <- round(weighted.mean(summary_nr_q4$q4_gujarat_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_haryana_nr <- round(weighted.mean(summary_nr_q4$q4_haryana_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_hp_nr <- round(weighted.mean(summary_nr_q4$q4_hp_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_jk_nr <- round(weighted.mean(summary_nr_q4$q4_jk_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_jharkhand_nr <- round(weighted.mean(summary_nr_q4$q4_jharkhand_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_karnataka_nr <- round(weighted.mean(summary_nr_q4$q4_karnataka_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_kerala_nr <- round(weighted.mean(summary_nr_q4$q4_kerala_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_ladakh_nr <- round(weighted.mean(summary_nr_q4$q4_ladakh_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_lakshadweep_nr <- round(weighted.mean(summary_nr_q4$q4_lakshadweep_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_mp_nr <- round(weighted.mean(summary_nr_q4$q4_mp_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_maharashtra_nr <- round(weighted.mean(summary_nr_q4$q4_maharashtra_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_manipur_nr <- round(weighted.mean(summary_nr_q4$q4_manipur_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_meghalaya_nr <- round(weighted.mean(summary_nr_q4$q4_meghalaya_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_mizoram_nr <- round(weighted.mean(summary_nr_q4$q4_mizoram_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_nagaland_nr <- round(weighted.mean(summary_nr_q4$q4_nagaland_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_delhi_nr <- round(weighted.mean(summary_nr_q4$q4_delhi_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_none_nr <- round(weighted.mean(summary_nr_q4$q4_none_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_odisha_nr <- round(weighted.mean(summary_nr_q4$q4_odisha_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_puducherry_nr <- round(weighted.mean(summary_nr_q4$q4_puducherry_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_punjab_nr <- round(weighted.mean(summary_nr_q4$q4_punjab_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_rajasthan_nr <- round(weighted.mean(summary_nr_q4$q4_rajasthan_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_sikkim_nr <- round(weighted.mean(summary_nr_q4$q4_sikkim_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_tn_nr <- round(weighted.mean(summary_nr_q4$q4_tn_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_telangana_nr <- round(weighted.mean(summary_nr_q4$q4_telangana_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_tripura_nr <- round(weighted.mean(summary_nr_q4$q4_tripura_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_up_nr <- round(weighted.mean(summary_nr_q4$q4_up_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_uttarakhand_nr <- round(weighted.mean(summary_nr_q4$q4_uttarakhand_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)
q4frnt_weighted_mean_wb_nr <- round(weighted.mean(summary_nr_q4$q4_wb_nr[summary_nr_q4$sdg_tier=="Frontrunner"], summary_nr_q4$sdg_weight[summary_nr_q4$sdg_tier=="Frontrunner"])*100,2)



#forming separate vectors
q4aspt_nr_plot <- c(q4aspt_weighted_mean_anislands_nr, q4aspt_weighted_mean_andhra_nr, q4aspt_weighted_mean_arunachal_nr, q4aspt_weighted_mean_assam_nr, q4aspt_weighted_mean_bihar_nr, q4aspt_weighted_mean_chandigarh_nr, q4aspt_weighted_mean_chhattisgarh_nr, q4aspt_weighted_mean_delhi_nr, q4aspt_weighted_mean_dnhdd_nr, q4aspt_weighted_mean_goa_nr, q4aspt_weighted_mean_gujarat_nr, q4aspt_weighted_mean_haryana_nr, q4aspt_weighted_mean_hp_nr, q4aspt_weighted_mean_jharkhand_nr, q4aspt_weighted_mean_jk_nr, q4aspt_weighted_mean_karnataka_nr, q4aspt_weighted_mean_kerala_nr, q4aspt_weighted_mean_ladakh_nr, q4aspt_weighted_mean_lakshadweep_nr, q4aspt_weighted_mean_maharashtra_nr, q4aspt_weighted_mean_manipur_nr, q4aspt_weighted_mean_meghalaya_nr, q4aspt_weighted_mean_mizoram_nr, q4aspt_weighted_mean_mp_nr, q4aspt_weighted_mean_nagaland_nr, q4aspt_weighted_mean_none_nr, q4aspt_weighted_mean_odisha_nr, q4aspt_weighted_mean_puducherry_nr, q4aspt_weighted_mean_punjab_nr, q4aspt_weighted_mean_rajasthan_nr, q4aspt_weighted_mean_sikkim_nr, q4aspt_weighted_mean_telangana_nr, q4aspt_weighted_mean_tn_nr, q4aspt_weighted_mean_tripura_nr, q4aspt_weighted_mean_up_nr, q4aspt_weighted_mean_uttarakhand_nr, q4aspt_weighted_mean_wb_nr)

q4perf_nr_plot <- c(q4perf_weighted_mean_anislands_nr, q4perf_weighted_mean_andhra_nr, q4perf_weighted_mean_arunachal_nr, q4perf_weighted_mean_assam_nr, q4perf_weighted_mean_bihar_nr, q4perf_weighted_mean_chandigarh_nr, q4perf_weighted_mean_chhattisgarh_nr, q4perf_weighted_mean_delhi_nr, q4perf_weighted_mean_dnhdd_nr, q4perf_weighted_mean_goa_nr, q4perf_weighted_mean_gujarat_nr, q4perf_weighted_mean_haryana_nr, q4perf_weighted_mean_hp_nr, q4perf_weighted_mean_jharkhand_nr, q4perf_weighted_mean_jk_nr, q4perf_weighted_mean_karnataka_nr, q4perf_weighted_mean_kerala_nr, q4perf_weighted_mean_ladakh_nr, q4perf_weighted_mean_lakshadweep_nr, q4perf_weighted_mean_maharashtra_nr, q4perf_weighted_mean_manipur_nr, q4perf_weighted_mean_meghalaya_nr, q4perf_weighted_mean_mizoram_nr, q4perf_weighted_mean_mp_nr, q4perf_weighted_mean_nagaland_nr, q4perf_weighted_mean_none_nr, q4perf_weighted_mean_odisha_nr, q4perf_weighted_mean_puducherry_nr, q4perf_weighted_mean_punjab_nr, q4perf_weighted_mean_rajasthan_nr, q4perf_weighted_mean_sikkim_nr, q4perf_weighted_mean_telangana_nr, q4perf_weighted_mean_tn_nr, q4perf_weighted_mean_tripura_nr, q4perf_weighted_mean_up_nr, q4perf_weighted_mean_uttarakhand_nr, q4perf_weighted_mean_wb_nr)

q4frnt_nr_plot <- c(q4frnt_weighted_mean_anislands_nr, q4frnt_weighted_mean_andhra_nr, q4frnt_weighted_mean_arunachal_nr, q4frnt_weighted_mean_assam_nr, q4frnt_weighted_mean_bihar_nr, q4frnt_weighted_mean_chandigarh_nr, q4frnt_weighted_mean_chhattisgarh_nr, q4frnt_weighted_mean_delhi_nr, q4frnt_weighted_mean_dnhdd_nr, q4frnt_weighted_mean_goa_nr, q4frnt_weighted_mean_gujarat_nr, q4frnt_weighted_mean_haryana_nr, q4frnt_weighted_mean_hp_nr, q4frnt_weighted_mean_jharkhand_nr, q4frnt_weighted_mean_jk_nr, q4frnt_weighted_mean_karnataka_nr, q4frnt_weighted_mean_kerala_nr, q4frnt_weighted_mean_ladakh_nr, q4frnt_weighted_mean_lakshadweep_nr, q4frnt_weighted_mean_maharashtra_nr, q4frnt_weighted_mean_manipur_nr, q4frnt_weighted_mean_meghalaya_nr, q4frnt_weighted_mean_mizoram_nr, q4frnt_weighted_mean_mp_nr, q4frnt_weighted_mean_nagaland_nr, q4frnt_weighted_mean_none_nr, q4frnt_weighted_mean_odisha_nr, q4frnt_weighted_mean_puducherry_nr, q4frnt_weighted_mean_punjab_nr, q4frnt_weighted_mean_rajasthan_nr, q4frnt_weighted_mean_sikkim_nr, q4frnt_weighted_mean_telangana_nr, q4frnt_weighted_mean_tn_nr, q4frnt_weighted_mean_tripura_nr, q4frnt_weighted_mean_up_nr, q4frnt_weighted_mean_uttarakhand_nr, q4frnt_weighted_mean_wb_nr)
...
#Invoking GGplot
##aspirant states
q4aspt_nr_plot_dta <- data.frame (q4aspt_nr_plot, q4full_plot_labels) 
q4aspt_nr_plot_dta$wrapped_labels = str_wrap(q4aspt_nr_plot_dta$q4full_plot_labels, width = 10)
q4aspt_nr_plot <- ggplot(q4aspt_nr_plot_dta, aes(x=q4full_plot_labels, y=q4aspt_nr_plot, fill = q4full_plot_labels)) +
  geom_bar(stat="identity", width= 1, show.legend = FALSE) + 
  geom_text(aes(label=paste0(sprintf("%0.2f", q4aspt_nr_plot),"%"), vjust=-0.3), size=3.5) +
  scale_y_continuous(limits = c(0,40)) +
  scale_x_discrete(labels = function(q4full_plot_labels) str_wrap(q4full_plot_labels, width = 30)) +
  labs(x="",y="Percent", title = "Aspirant States")+
  newtheme
...
##Performer States
q4perf_nr_plot_dta <- data.frame (q4perf_nr_plot, q4full_plot_labels) 
q4perf_nr_plot_dta$wrapped_labels = str_wrap(q4perf_nr_plot_dta$q4full_plot_labels, width = 10)
q4perf_nr_plot <- ggplot(q4perf_nr_plot_dta, aes(x=q4full_plot_labels, y=q4perf_nr_plot, fill = q4full_plot_labels)) +
  geom_bar(stat="identity", width= 1, show.legend = FALSE) + 
  geom_text(aes(label=paste0(sprintf("%0.2f", q4perf_nr_plot),"%"), vjust=-0.3), size=3.5) +
  scale_y_continuous(limits = c(0,40)) +
  scale_x_discrete(labels = function(q4full_plot_labels) str_wrap(q4full_plot_labels, width = 30)) +
  labs(x="",y="Percent", title = "Performer States")+
  newtheme
...
##Frontrunner States
q4frnt_nr_plot_dta <- data.frame (q4frnt_nr_plot, q4full_plot_labels) 
q4frnt_nr_plot_dta$wrapped_labels = str_wrap(q4frnt_nr_plot_dta$q4full_plot_labels, width = 10)
q4frnt_nr_plot <- ggplot(q4frnt_nr_plot_dta, aes(x=q4full_plot_labels, y=q4frnt_nr_plot, fill = q4full_plot_labels)) +
  geom_bar(stat="identity", width= 1, show.legend = FALSE) + 
  geom_text(aes(label=paste0(sprintf("%0.2f", q4frnt_nr_plot),"%"), vjust=-0.3), size=3.5) +
  scale_y_continuous(limits = c(0,40)) +
  scale_x_discrete(labels = function(q4full_plot_labels) str_wrap(q4full_plot_labels, width = 30)) +
  labs(x="",y="Percent", title = "Frontrunner States")+
  newtheme
...
#Stacking all plots vertically
grid.newpage()
grid.draw(rbind(ggplotGrob(q4aspt_nr_plot), ggplotGrob(q4perf_nr_plot), ggplotGrob(q4frnt_nr_plot), size = "last"))
```
\newpage
...
117 officers out of 452 felt that their own state was under-appreciated for its successful pandemic management. These officers may be biased in their response. The following graph excludes all such potentially biased responses for question 6.

\vspace{6mm}
```{r, echo = FALSE, fig.height=10,fig.width=18, warning=FALSE}
q6full_mean_anislands <- round(weighted.mean(summary_q6_unbiased$q6_anislands, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_andhra <- round(weighted.mean(summary_q6_unbiased$q6_andhra, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_arunachal <- round(weighted.mean(summary_q6_unbiased$q6_arunachal, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_assam <- round(weighted.mean(summary_q6_unbiased$q6_assam, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_bihar <- round(weighted.mean(summary_q6_unbiased$q6_bihar, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_chandigarh <- round(weighted.mean(summary_q6_unbiased$q6_chandigarh, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_chhattisgarh <- round(weighted.mean(summary_q6_unbiased$q6_chhattisgarh, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_dnhdd <- round(weighted.mean(summary_q6_unbiased$q6_dnhdd, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_goa <- round(weighted.mean(summary_q6_unbiased$q6_goa, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_gujarat <- round(weighted.mean(summary_q6_unbiased$q6_gujarat, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_haryana <- round(weighted.mean(summary_q6_unbiased$q6_haryana, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_hp <- round(weighted.mean(summary_q6_unbiased$q6_hp, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_jk <- round(weighted.mean(summary_q6_unbiased$q6_jk, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_jharkhand <- round(weighted.mean(summary_q6_unbiased$q6_jharkhand, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_karnataka <- round(weighted.mean(summary_q6_unbiased$q6_karnataka, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_kerala <- round(weighted.mean(summary_q6_unbiased$q6_kerala, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_ladakh <- round(weighted.mean(summary_q6_unbiased$q6_ladakh, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_lakshadweep <- round(weighted.mean(summary_q6_unbiased$q6_lakshadweep, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_mp <- round(weighted.mean(summary_q6_unbiased$q6_mp, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_maharashtra <- round(weighted.mean(summary_q6_unbiased$q6_maharashtra, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_manipur <- round(weighted.mean(summary_q6_unbiased$q6_manipur, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_meghalaya <- round(weighted.mean(summary_q6_unbiased$q6_meghalaya, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_mizoram <- round(weighted.mean(summary_q6_unbiased$q6_mizoram, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_nagaland <- round(weighted.mean(summary_q6_unbiased$q6_nagaland, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_delhi <- round(weighted.mean(summary_q6_unbiased$q6_delhi, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_none <- round(weighted.mean(summary_q6_unbiased$q6_none, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_odisha <- round(weighted.mean(summary_q6_unbiased$q6_odisha, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_puducherry <- round(weighted.mean(summary_q6_unbiased$q6_puducherry, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_punjab <- round(weighted.mean(summary_q6_unbiased$q6_punjab, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_rajasthan <- round(weighted.mean(summary_q6_unbiased$q6_rajasthan, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_sikkim <- round(weighted.mean(summary_q6_unbiased$q6_sikkim, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_tn <- round(weighted.mean(summary_q6_unbiased$q6_tn, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_telangana <- round(weighted.mean(summary_q6_unbiased$q6_telangana, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_tripura <- round(weighted.mean(summary_q6_unbiased$q6_tripura, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_up <- round(weighted.mean(summary_q6_unbiased$q6_up, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_uttarakhand <- round(weighted.mean(summary_q6_unbiased$q6_uttarakhand, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
q6full_mean_wb <- round(weighted.mean(summary_q6_unbiased$q6_wb, summary_q6_unbiased$state_weight, na.rm = TRUE)*100,2)
...
100 officers out of the 248 officers who joined the IAS in 2012 or later felt that their own state has been under-appreciated for its successful pandemic management. 71 of the 204 officers who joined the IAS in 2011 or before, and 24 of 96 former officers were similarly biased in their responses to this question. The following graphs exclude these potentially biased responses. 
...
##Performers
q6perf_weighted_mean_anislands <- round(weighted.mean(summary_q6_unbiased$q6_anislands[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_andhra <- round(weighted.mean(summary_q6_unbiased$q6_andhra[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_arunachal <- round(weighted.mean(summary_q6_unbiased$q6_arunachal[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_assam <- round(weighted.mean(summary_q6_unbiased$q6_assam[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_bihar <- round(weighted.mean(summary_q6_unbiased$q6_bihar[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_chandigarh <- round(weighted.mean(summary_q6_unbiased$q6_chandigarh[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_chhattisgarh <- round(weighted.mean(summary_q6_unbiased$q6_chhattisgarh[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_dnhdd <- round(weighted.mean(summary_q6_unbiased$q6_dnhdd[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_goa <- round(weighted.mean(summary_q6_unbiased$q6_goa[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_gujarat <- round(weighted.mean(summary_q6_unbiased$q6_gujarat[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_haryana <- round(weighted.mean(summary_q6_unbiased$q6_haryana[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_hp <- round(weighted.mean(summary_q6_unbiased$q6_hp[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_jk <- round(weighted.mean(summary_q6_unbiased$q6_jk[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_jharkhand <- round(weighted.mean(summary_q6_unbiased$q6_jharkhand[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_karnataka <- round(weighted.mean(summary_q6_unbiased$q6_karnataka[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_kerala <- round(weighted.mean(summary_q6_unbiased$q6_kerala[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_ladakh <- round(weighted.mean(summary_q6_unbiased$q6_ladakh[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_lakshadweep <- round(weighted.mean(summary_q6_unbiased$q6_lakshadweep[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_mp <- round(weighted.mean(summary_q6_unbiased$q6_mp[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_maharashtra <- round(weighted.mean(summary_q6_unbiased$q6_maharashtra[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_manipur <- round(weighted.mean(summary_q6_unbiased$q6_manipur[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_meghalaya <- round(weighted.mean(summary_q6_unbiased$q6_meghalaya[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_mizoram <- round(weighted.mean(summary_q6_unbiased$q6_mizoram[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_nagaland <- round(weighted.mean(summary_q6_unbiased$q6_nagaland[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_delhi <- round(weighted.mean(summary_q6_unbiased$q6_delhi[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_none <- round(weighted.mean(summary_q6_unbiased$q6_none[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_odisha <- round(weighted.mean(summary_q6_unbiased$q6_odisha[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_puducherry <- round(weighted.mean(summary_q6_unbiased$q6_puducherry[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_punjab <- round(weighted.mean(summary_q6_unbiased$q6_punjab[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_rajasthan <- round(weighted.mean(summary_q6_unbiased$q6_rajasthan[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_sikkim <- round(weighted.mean(summary_q6_unbiased$q6_sikkim[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_tn <- round(weighted.mean(summary_q6_unbiased$q6_tn[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_telangana <- round(weighted.mean(summary_q6_unbiased$q6_telangana[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_tripura <- round(weighted.mean(summary_q6_unbiased$q6_tripura[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_up <- round(weighted.mean(summary_q6_unbiased$q6_up[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_uttarakhand <- round(weighted.mean(summary_q6_unbiased$q6_uttarakhand[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
q6perf_weighted_mean_wb <- round(weighted.mean(summary_q6_unbiased$q6_wb[summary_q6_unbiased$sdg_tier=="Performer"], summary_q6_unbiased$sdg_weight[summary_q6_unbiased$sdg_tier=="Performer"], na.rm = TRUE)*100,2)
...
States_Q4 <- c("Andaman and Nicobar Islands", "Andhra Pradesh", "Arunachal Pradesh", "Assam", "Bihar", "Chandigarh", "Chattisgarh", "NCT of Delhi","Dadra and Nagar Haveli and Daman and Dui", "Goa", "Gujarat", "Haryana", "Himachal Pradesh", "Jharkhand", "Jammu and Kashmir", "Karnataka", "Kerala", "Ladakh", "Lakshadweep Islands", "Maharashtra", "Manipur", "Meghalaya", "Mizoram", "Madhya Pradesh", "Nagaland","No state has performed well", "Odisha", "Puducherry", "Punjab", "Rajasthan", "Sikkim", "Telangana", "Tamil Nadu", "Tripura", "Uttar Pradesh", "Uttarakhand", "West Bengal")

#Question 4, aggregate
q4full_plot1 <- c(q4full_mean_anislands, q4full_mean_andhra, q4full_mean_arunachal, q4full_mean_assam, q4full_mean_bihar, q4full_mean_chandigarh, q4full_mean_chhattisgarh, q4full_mean_delhi, q4full_mean_dnhdd, q4full_mean_goa, q4full_mean_gujarat, q4full_mean_haryana, q4full_mean_hp, q4full_mean_jharkhand, q4full_mean_jk, q4full_mean_karnataka, q4full_mean_kerala, q4full_mean_ladakh, q4full_mean_lakshadweep, q4full_mean_maharashtra, q4full_mean_manipur, q4full_mean_meghalaya, q4full_mean_mizoram, q4full_mean_mp, q4full_mean_nagaland, q4full_mean_none, q4full_mean_odisha, q4full_mean_puducherry, q4full_mean_punjab, q4full_mean_rajasthan, q4full_mean_sikkim, q4full_mean_telangana, q4full_mean_tn, q4full_mean_tripura, q4full_mean_up, q4full_mean_uttarakhand, q4full_mean_wb)

q4_plot_dta1 <- data.frame (States_Q4, q4full_plot1) %>% 
arrange (desc(q4full_plot1)) %>% 
rename ("Aggregate Percentages (Q4)" = q4full_plot1)

States_Q4 <- c("Andaman and Nicobar Islands", "Andhra Pradesh", "Arunachal Pradesh", "Assam", "Bihar", "Chandigarh", "Chattisgarh", "NCT of Delhi","Dadra and Nagar Haveli and Daman and Dui", "Goa", "Gujarat", "Haryana", "Himachal Pradesh", "Jharkhand", "Jammu and Kashmir", "Karnataka", "Kerala", "Ladakh", "Lakshadweep Islands", "Maharashtra", "Manipur", "Meghalaya", "Mizoram", "Madhya Pradesh", "Nagaland","No state has performed well", "Odisha", "Puducherry", "Punjab", "Rajasthan", "Sikkim", "Telangana", "Tamil Nadu", "Tripura", "Uttar Pradesh", "Uttarakhand", "West Bengal")

#Question 4, aggregate (accounting for officer's bias)
q4full_nr_plot <- c(q4full_mean_anislands_nr, q4full_mean_andhra_nr, q4full_mean_arunachal_nr, q4full_mean_assam_nr, q4full_mean_bihar_nr, q4full_mean_chandigarh_nr, q4full_mean_chhattisgarh_nr, q4full_mean_delhi_nr, q4full_mean_dnhdd_nr, q4full_mean_goa_nr, q4full_mean_gujarat_nr, q4full_mean_haryana_nr, q4full_mean_hp_nr, q4full_mean_jharkhand_nr, q4full_mean_jk_nr, q4full_mean_karnataka_nr, q4full_mean_kerala_nr, q4full_mean_ladakh_nr, q4full_mean_lakshadweep_nr, q4full_mean_maharashtra_nr, q4full_mean_manipur_nr, q4full_mean_meghalaya_nr, q4full_mean_mizoram_nr, q4full_mean_mp_nr, q4full_mean_nagaland_nr, q4full_mean_none_nr, q4full_mean_odisha_nr, q4full_mean_puducherry_nr, q4full_mean_punjab_nr, q4full_mean_rajasthan_nr, q4full_mean_sikkim_nr, q4full_mean_telangana_nr, q4full_mean_tn_nr, q4full_mean_tripura_nr, q4full_mean_up_nr, q4full_mean_uttarakhand_nr, q4full_mean_wb_nr)

q4_plot_dta2 <- data.frame (States_Q4, q4full_nr_plot) %>% 
arrange (desc(q4full_nr_plot)) %>% 
rename ("Q4 Aggregate Percentages (accounting for bias)" = q4full_nr_plot)

#Question 4, Disaggreaged by Age
q4young_plot1 <- c(q4young_mean_anislands, q4young_mean_andhra, q4young_mean_arunachal, q4young_mean_assam, q4young_mean_bihar, q4young_mean_chandigarh, q4young_mean_chhattisgarh, q4young_mean_delhi, q4young_mean_dnhdd, q4young_mean_goa, q4young_mean_gujarat, q4young_mean_haryana, q4young_mean_hp, q4young_mean_jharkhand, q4young_mean_jk, q4young_mean_karnataka, q4young_mean_kerala, q4young_mean_ladakh, q4young_mean_lakshadweep, q4young_mean_maharashtra, q4young_mean_manipur, q4young_mean_meghalaya, q4young_mean_mizoram, q4young_mean_mp, q4young_mean_nagaland, q4young_mean_none, q4young_mean_odisha, q4young_mean_puducherry, q4young_mean_punjab, q4young_mean_rajasthan, q4young_mean_sikkim, q4young_mean_telangana, q4young_mean_tn, q4young_mean_tripura, q4young_mean_up, q4young_mean_uttarakhand, q4young_mean_wb)

q4young_plot_dta1 <- data.frame (States_Q4, q4young_plot1) 

q4old_plot1 <- c(q4old_mean_anislands, q4old_mean_andhra, q4old_mean_arunachal, q4old_mean_assam, q4old_mean_bihar, q4old_mean_chandigarh, q4old_mean_chhattisgarh, q4old_mean_delhi, q4old_mean_dnhdd, q4old_mean_goa, q4old_mean_gujarat, q4old_mean_haryana, q4old_mean_hp, q4old_mean_jharkhand, q4old_mean_jk, q4old_mean_karnataka, q4old_mean_kerala, q4old_mean_ladakh, q4old_mean_lakshadweep, q4old_mean_maharashtra, q4old_mean_manipur, q4old_mean_meghalaya, q4old_mean_mizoram, q4old_mean_mp, q4old_mean_nagaland, q4old_mean_none, q4old_mean_odisha, q4old_mean_puducherry, q4old_mean_punjab, q4old_mean_rajasthan, q4old_mean_sikkim, q4old_mean_telangana, q4old_mean_tn, q4old_mean_tripura, q4old_mean_up, q4old_mean_uttarakhand, q4old_mean_wb)

q4old_plot_dta1 <- data.frame (q4old_plot1)

q4former_plot1 <- c(q4former_mean_anislands, q4former_mean_andhra, q4former_mean_arunachal, q4former_mean_assam, q4former_mean_bihar, q4former_mean_chandigarh, q4former_mean_chhattisgarh, q4former_mean_delhi, q4former_mean_dnhdd, q4former_mean_goa, q4former_mean_gujarat, q4former_mean_haryana, q4former_mean_hp, q4former_mean_jharkhand, q4former_mean_jk, q4former_mean_karnataka, q4former_mean_kerala, q4former_mean_ladakh, q4former_mean_lakshadweep, q4former_mean_maharashtra, q4former_mean_manipur, q4former_mean_meghalaya, q4former_mean_mizoram, q4former_mean_mp, q4former_mean_nagaland, q4former_mean_none, q4former_mean_odisha, q4former_mean_puducherry, q4former_mean_punjab, q4former_mean_rajasthan, q4former_mean_sikkim, q4former_mean_telangana, q4former_mean_tn, q4former_mean_tripura, q4former_mean_up, q4former_mean_uttarakhand, q4former_mean_wb)

q4former_plot_dta1 <- data.frame (q4former_plot1)

q4_age_dta <- cbind (q4young_plot_dta1, q4old_plot_dta1, q4former_plot_dta1) %>% 
arrange (desc(q4old_plot_dta1)) %>% 
rename ("Percentages (Q4 Young)" = q4young_plot1) %>% 
rename ("Percentages (Q4 Old)" = q4old_plot1) %>% 
rename ("Percentages (Q4 Former)" = q4former_plot1)

#Question 4, Disaggregated by age (accounting for bias)

q4young_nr_plot <- c(q4young_mean_anislands_nr, q4young_mean_andhra_nr, q4young_mean_arunachal_nr, q4young_mean_assam_nr, q4young_mean_bihar_nr, q4young_mean_chandigarh_nr, q4young_mean_chhattisgarh_nr, q4young_mean_delhi_nr, q4young_mean_dnhdd_nr, q4young_mean_goa_nr, q4young_mean_gujarat_nr, q4young_mean_haryana_nr, q4young_mean_hp_nr, q4young_mean_jharkhand_nr, q4young_mean_jk_nr, q4young_mean_karnataka_nr, q4young_mean_kerala_nr, q4young_mean_ladakh_nr, q4young_mean_lakshadweep_nr, q4young_mean_maharashtra_nr, q4young_mean_manipur_nr, q4young_mean_meghalaya_nr, q4young_mean_mizoram_nr, q4young_mean_mp_nr, q4young_mean_nagaland_nr, q4young_mean_none_nr, q4young_mean_odisha_nr, q4young_mean_puducherry_nr, q4young_mean_punjab_nr, q4young_mean_rajasthan_nr, q4young_mean_sikkim_nr, q4young_mean_telangana_nr, q4young_mean_tn_nr, q4young_mean_tripura_nr, q4young_mean_up_nr, q4young_mean_uttarakhand_nr, q4young_mean_wb_nr)

q4young_plot_dta2 <- data.frame (States_Q4, q4young_nr_plot) 

q4old_nr_plot <- c(q4old_mean_anislands_nr, q4old_mean_andhra_nr, q4old_mean_arunachal_nr, q4old_mean_assam_nr, q4old_mean_bihar_nr, q4old_mean_chandigarh_nr, q4old_mean_chhattisgarh_nr, q4old_mean_delhi_nr, q4old_mean_dnhdd_nr, q4old_mean_goa_nr, q4old_mean_gujarat_nr, q4old_mean_haryana_nr, q4old_mean_hp_nr, q4old_mean_jharkhand_nr, q4old_mean_jk_nr, q4old_mean_karnataka_nr, q4old_mean_kerala_nr, q4old_mean_ladakh_nr, q4old_mean_lakshadweep_nr, q4old_mean_maharashtra_nr, q4old_mean_manipur_nr, q4old_mean_meghalaya_nr, q4old_mean_mizoram_nr, q4old_mean_mp_nr, q4old_mean_nagaland_nr, q4old_mean_none_nr, q4old_mean_odisha_nr, q4old_mean_puducherry_nr, q4old_mean_punjab_nr, q4old_mean_rajasthan_nr, q4old_mean_sikkim_nr, q4old_mean_telangana_nr, q4old_mean_tn_nr, q4old_mean_tripura_nr, q4old_mean_up_nr, q4old_mean_uttarakhand_nr, q4old_mean_wb_nr)

q4old_plot_dta2 <- data.frame (q4old_nr_plot)

q4former_nr_plot <- c(q4former_mean_anislands_nr, q4former_mean_andhra_nr, q4former_mean_arunachal_nr, q4former_mean_assam_nr, q4former_mean_bihar_nr, q4former_mean_chandigarh_nr, q4former_mean_chhattisgarh_nr, q4former_mean_delhi_nr, q4former_mean_dnhdd_nr, q4former_mean_goa_nr, q4former_mean_gujarat_nr, q4former_mean_haryana_nr, q4former_mean_hp_nr, q4former_mean_jharkhand_nr, q4former_mean_jk_nr, q4former_mean_karnataka_nr, q4former_mean_kerala_nr, q4former_mean_ladakh_nr, q4former_mean_lakshadweep_nr, q4former_mean_maharashtra_nr, q4former_mean_manipur_nr, q4former_mean_meghalaya_nr, q4former_mean_mizoram_nr, q4former_mean_mp_nr, q4former_mean_nagaland_nr, q4former_mean_none_nr, q4former_mean_odisha_nr, q4former_mean_puducherry_nr, q4former_mean_punjab_nr, q4former_mean_rajasthan_nr, q4former_mean_sikkim_nr, q4former_mean_telangana_nr, q4former_mean_tn_nr, q4former_mean_tripura_nr, q4former_mean_up_nr, q4former_mean_uttarakhand_nr, q4former_mean_wb_nr)

q4former_plot_dta2 <- data.frame (q4former_nr_plot)

q4_age_dta2 <- cbind (q4young_plot_dta2, q4old_plot_dta2, q4former_plot_dta2) %>% 
arrange (desc(q4old_plot_dta2)) %>% 
rename ("Q4 Percentages (Young and unbiased)" = q4young_nr_plot) %>% 
rename ("Q4 Percentages (Old and unbiased)" = q4old_nr_plot) %>% 
rename ("Q4 Percentages (Former and unbiased)" = q4former_nr_plot)

#Question 4, Disaggregated by SDG

q4aspt_plot1 <- c(q4aspt_weighted_mean_anislands, q4aspt_weighted_mean_andhra, q4aspt_weighted_mean_arunachal, q4aspt_weighted_mean_assam, q4aspt_weighted_mean_bihar, q4aspt_weighted_mean_chandigarh, q4aspt_weighted_mean_chhattisgarh, q4aspt_weighted_mean_delhi, q4aspt_weighted_mean_dnhdd, q4aspt_weighted_mean_goa, q4aspt_weighted_mean_gujarat, q4aspt_weighted_mean_haryana, q4aspt_weighted_mean_hp, q4aspt_weighted_mean_jharkhand, q4aspt_weighted_mean_jk, q4aspt_weighted_mean_karnataka, q4aspt_weighted_mean_kerala, q4aspt_weighted_mean_ladakh, q4aspt_weighted_mean_lakshadweep, q4aspt_weighted_mean_maharashtra, q4aspt_weighted_mean_manipur, q4aspt_weighted_mean_meghalaya, q4aspt_weighted_mean_mizoram, q4aspt_weighted_mean_mp, q4aspt_weighted_mean_nagaland, q4aspt_weighted_mean_none, q4aspt_weighted_mean_odisha, q4aspt_weighted_mean_puducherry, q4aspt_weighted_mean_punjab, q4aspt_weighted_mean_rajasthan, q4aspt_weighted_mean_sikkim, q4aspt_weighted_mean_telangana, q4aspt_weighted_mean_tn, q4aspt_weighted_mean_tripura, q4aspt_weighted_mean_up, q4aspt_weighted_mean_uttarakhand, q4aspt_weighted_mean_wb)

q4perf_plot1 <- c(q4perf_weighted_mean_anislands, q4perf_weighted_mean_andhra, q4perf_weighted_mean_arunachal, q4perf_weighted_mean_assam, q4perf_weighted_mean_bihar, q4perf_weighted_mean_chandigarh, q4perf_weighted_mean_chhattisgarh, q4perf_weighted_mean_delhi, q4perf_weighted_mean_dnhdd, q4perf_weighted_mean_goa, q4perf_weighted_mean_gujarat, q4perf_weighted_mean_haryana, q4perf_weighted_mean_hp, q4perf_weighted_mean_jharkhand, q4perf_weighted_mean_jk, q4perf_weighted_mean_karnataka, q4perf_weighted_mean_kerala, q4perf_weighted_mean_ladakh, q4perf_weighted_mean_lakshadweep, q4perf_weighted_mean_maharashtra, q4perf_weighted_mean_manipur, q4perf_weighted_mean_meghalaya, q4perf_weighted_mean_mizoram, q4perf_weighted_mean_mp, q4perf_weighted_mean_nagaland, q4perf_weighted_mean_none, q4perf_weighted_mean_odisha, q4perf_weighted_mean_puducherry, q4perf_weighted_mean_punjab, q4perf_weighted_mean_rajasthan, q4perf_weighted_mean_sikkim, q4perf_weighted_mean_telangana, q4perf_weighted_mean_tn, q4perf_weighted_mean_tripura, q4perf_weighted_mean_up, q4perf_weighted_mean_uttarakhand, q4perf_weighted_mean_wb)

q4frnt_plot1 <- c(q4frnt_weighted_mean_anislands, q4frnt_weighted_mean_andhra, q4frnt_weighted_mean_arunachal, q4frnt_weighted_mean_assam, q4frnt_weighted_mean_bihar, q4frnt_weighted_mean_chandigarh, q4frnt_weighted_mean_chhattisgarh, q4frnt_weighted_mean_delhi, q4frnt_weighted_mean_dnhdd, q4frnt_weighted_mean_goa, q4frnt_weighted_mean_gujarat, q4frnt_weighted_mean_haryana, q4frnt_weighted_mean_hp, q4frnt_weighted_mean_jharkhand, q4frnt_weighted_mean_jk, q4frnt_weighted_mean_karnataka, q4frnt_weighted_mean_kerala, q4frnt_weighted_mean_ladakh, q4frnt_weighted_mean_lakshadweep, q4frnt_weighted_mean_maharashtra, q4frnt_weighted_mean_manipur, q4frnt_weighted_mean_meghalaya, q4frnt_weighted_mean_mizoram, q4frnt_weighted_mean_mp, q4frnt_weighted_mean_nagaland, q4frnt_weighted_mean_none, q4frnt_weighted_mean_odisha, q4frnt_weighted_mean_puducherry, q4frnt_weighted_mean_punjab, q4frnt_weighted_mean_rajasthan, q4frnt_weighted_mean_sikkim, q4frnt_weighted_mean_telangana, q4frnt_weighted_mean_tn, q4frnt_weighted_mean_tripura, q4frnt_weighted_mean_up, q4frnt_weighted_mean_uttarakhand, q4frnt_weighted_mean_wb)

q4aspt_plot_dta1 <- data.frame (States_Q4, q4aspt_plot1) 
q4perf_plot_dta1 <- data.frame (q4perf_plot1)
q4frnt_plot_dta1 <- data.frame (q4frnt_plot1)

q4_sdg_dta <- cbind (q4aspt_plot_dta1, q4perf_plot_dta1, q4frnt_plot_dta1) %>% 
arrange (desc(q4perf_plot_dta1)) %>% 
rename ("Percentages (Q4 Aspirant)" = q4aspt_plot1) %>% 
rename ("Percentages (Q4 Performer)" = q4perf_plot1) %>% 
rename ("Percentages (Q4 Frontrunner)" = q4frnt_plot1)

#Question 4: Disaggregated by SDG (accounting for bias)
...
q4aspt_plot_dta2 <- data.frame (States_Q4, q4aspt_nr_plot) 
q4perf_plot_dta2 <- data.frame (q4perf_nr_plot)
q4frnt_plot_dta2 <- data.frame (q4frnt_nr_plot)

q4_sdg_dta2 <- cbind (q4aspt_plot_dta2, q4perf_plot_dta2, q4frnt_plot_dta2) %>% 
arrange (desc(q4perf_plot_dta2)) %>% 
rename ("Q4 Percentages (Aspirant, Unbiased)" = q4aspt_nr_plot) %>% 
rename ("Q4 Percentages (Performer, Unbiased)" = q4perf_nr_plot) %>% 
rename ("Q4 Percentages (Frontrunner, Unbiased)" = q4frnt_nr_plot)

write.table (q4_plot_dta1, "/Users/gurkirat/Documents/GitHub/ias-release/q4_plot_dta1.txt", sep = "\t")
write.table (q4_age_dta, "/Users/gurkirat/Documents/GitHub/ias-release/q4_age_dta.txt", sep = "\t")
write.table (q4_sdg_dta, "/Users/gurkirat/Documents/GitHub/ias-release/q4_sdg_dta.txt", sep = "\t")
write.table (q4_plot_dta2, "/Users/gurkirat/Documents/GitHub/ias-release/q4_plot_dta2.txt", sep = "\t")
write.table (q4_age_dta2, "/Users/gurkirat/Documents/GitHub/ias-release/q4_age_dta2.txt", sep = "\t")
write.table (q4_sdg_dta2, "/Users/gurkirat/Documents/GitHub/ias-release/q4_sdg_dta2.txt", sep = "\t")
```

```{r}
#Tables for question 5
States_Q5 <- c("Andaman and Nicobar Islands", "Andhra Pradesh", "Arunachal Pradesh", "Assam", "Bihar", "Chandigarh", "Chattisgarh", "NCT of Delhi","Dadra and Nagar Haveli and Daman and Dui", "Goa", "Gujarat", "Haryana", "Himachal Pradesh", "Jharkhand", "Jammu and Kashmir", "Karnataka", "Kerala", "Ladakh", "Lakshadweep Islands", "Maharashtra", "Manipur", "Meghalaya", "Mizoram", "Madhya Pradesh", "Nagaland","No state has performed poorly", "Odisha", "Puducherry", "Punjab", "Rajasthan", "Sikkim", "Telangana", "Tamil Nadu", "Tripura", "Uttar Pradesh", "Uttarakhand", "West Bengal")

#Question 5, aggregate
q5full_plot1 <- c(q5full_mean_anislands, q5full_mean_andhra, q5full_mean_arunachal, q5full_mean_assam, q5full_mean_bihar, q5full_mean_chandigarh, q5full_mean_chhattisgarh, q5full_mean_delhi, q5full_mean_dnhdd, q5full_mean_goa, q5full_mean_gujarat, q5full_mean_haryana, q5full_mean_hp, q5full_mean_jharkhand, q5full_mean_jk, q5full_mean_karnataka, q5full_mean_kerala, q5full_mean_ladakh, q5full_mean_lakshadweep, q5full_mean_maharashtra, q5full_mean_manipur, q5full_mean_meghalaya, q5full_mean_mizoram, q5full_mean_mp, q5full_mean_nagaland, q5full_mean_none, q5full_mean_odisha, q5full_mean_puducherry, q5full_mean_punjab, q5full_mean_rajasthan, q5full_mean_sikkim, q5full_mean_telangana, q5full_mean_tn, q5full_mean_tripura, q5full_mean_up, q5full_mean_uttarakhand, q5full_mean_wb)

q5_plot_dta1 <- data.frame (States_Q5, q5full_plot1) %>% 
arrange (desc(q5full_plot1)) %>% 
rename ("Aggregate Percentages (Q5)" = q5full_plot1)

#Question 4, Disaggreaged by Age
q5young_plot1 <- c(q5young_mean_anislands, q5young_mean_andhra, q5young_mean_arunachal, q5young_mean_assam, q5young_mean_bihar, q5young_mean_chandigarh, q5young_mean_chhattisgarh, q5young_mean_delhi, q5young_mean_dnhdd, q5young_mean_goa, q5young_mean_gujarat, q5young_mean_haryana, q5young_mean_hp, q5young_mean_jharkhand, q5young_mean_jk, q5young_mean_karnataka, q5young_mean_kerala, q5young_mean_ladakh, q5young_mean_lakshadweep, q5young_mean_maharashtra, q5young_mean_manipur, q5young_mean_meghalaya, q5young_mean_mizoram, q5young_mean_mp, q5young_mean_nagaland, q5young_mean_none, q5young_mean_odisha, q5young_mean_puducherry, q5young_mean_punjab, q5young_mean_rajasthan, q5young_mean_sikkim, q5young_mean_telangana, q5young_mean_tn, q5young_mean_tripura, q5young_mean_up, q5young_mean_uttarakhand, q5young_mean_wb)

q5young_plot_dta1 <- data.frame (States_Q5, q5young_plot1) 

q5old_plot1 <- c(q5old_mean_anislands, q5old_mean_andhra, q5old_mean_arunachal, q5old_mean_assam, q5old_mean_bihar, q5old_mean_chandigarh, q5old_mean_chhattisgarh, q5old_mean_delhi, q5old_mean_dnhdd, q5old_mean_goa, q5old_mean_gujarat, q5old_mean_haryana, q5old_mean_hp, q5old_mean_jharkhand, q5old_mean_jk, q5old_mean_karnataka, q5old_mean_kerala, q5old_mean_ladakh, q5old_mean_lakshadweep, q5old_mean_maharashtra, q5old_mean_manipur, q5old_mean_meghalaya, q5old_mean_mizoram, q5old_mean_mp, q5old_mean_nagaland, q5old_mean_none, q5old_mean_odisha, q5old_mean_puducherry, q5old_mean_punjab, q5old_mean_rajasthan, q5old_mean_sikkim, q5old_mean_telangana, q5old_mean_tn, q5old_mean_tripura, q5old_mean_up, q5old_mean_uttarakhand, q5old_mean_wb)

q5old_plot_dta1 <- data.frame (q5old_plot1)

q5former_plot1 <- c(q5former_mean_anislands, q5former_mean_andhra, q5former_mean_arunachal, q5former_mean_assam, q5former_mean_bihar, q5former_mean_chandigarh, q5former_mean_chhattisgarh, q5former_mean_delhi, q5former_mean_dnhdd, q5former_mean_goa, q5former_mean_gujarat, q5former_mean_haryana, q5former_mean_hp, q5former_mean_jharkhand, q5former_mean_jk, q5former_mean_karnataka, q5former_mean_kerala, q5former_mean_ladakh, q5former_mean_lakshadweep, q5former_mean_maharashtra, q5former_mean_manipur, q5former_mean_meghalaya, q5former_mean_mizoram, q5former_mean_mp, q5former_mean_nagaland, q5former_mean_none, q5former_mean_odisha, q5former_mean_puducherry, q5former_mean_punjab, q5former_mean_rajasthan, q5former_mean_sikkim, q5former_mean_telangana, q5former_mean_tn, q5former_mean_tripura, q5former_mean_up, q5former_mean_uttarakhand, q5former_mean_wb)

q5former_plot_dta1 <- data.frame (q5former_plot1)

q5_age_dta <- cbind (q5young_plot_dta1, q5old_plot_dta1, q5former_plot_dta1) %>% 
arrange (desc(q5old_plot_dta1)) %>% 
rename ("Percentages (Q5 Young)" = q5young_plot1) %>% 
rename ("Percentages (Q5 Old)" = q5old_plot1) %>% 
rename ("Percentages (Q5 Former)" = q5former_plot1)

#Question 5, Disaggregated by SDG
q5aspt_plot1 <- c(q5aspt_weighted_mean_anislands, q5aspt_weighted_mean_andhra, q5aspt_weighted_mean_arunachal, q5aspt_weighted_mean_assam, q5aspt_weighted_mean_bihar, q5aspt_weighted_mean_chandigarh, q5aspt_weighted_mean_chhattisgarh, q5aspt_weighted_mean_delhi, q5aspt_weighted_mean_dnhdd, q5aspt_weighted_mean_goa, q5aspt_weighted_mean_gujarat, q5aspt_weighted_mean_haryana, q5aspt_weighted_mean_hp, q5aspt_weighted_mean_jharkhand, q5aspt_weighted_mean_jk, q5aspt_weighted_mean_karnataka, q5aspt_weighted_mean_kerala, q5aspt_weighted_mean_ladakh, q5aspt_weighted_mean_lakshadweep, q5aspt_weighted_mean_maharashtra, q5aspt_weighted_mean_manipur, q5aspt_weighted_mean_meghalaya, q5aspt_weighted_mean_mizoram, q5aspt_weighted_mean_mp, q5aspt_weighted_mean_nagaland, q5aspt_weighted_mean_none, q5aspt_weighted_mean_odisha, q5aspt_weighted_mean_puducherry, q5aspt_weighted_mean_punjab, q5aspt_weighted_mean_rajasthan, q5aspt_weighted_mean_sikkim, q5aspt_weighted_mean_telangana, q5aspt_weighted_mean_tn, q5aspt_weighted_mean_tripura, q5aspt_weighted_mean_up, q5aspt_weighted_mean_uttarakhand, q5aspt_weighted_mean_wb)

q5perf_plot1 <- c(q5perf_weighted_mean_anislands, q5perf_weighted_mean_andhra, q5perf_weighted_mean_arunachal, q5perf_weighted_mean_assam, q5perf_weighted_mean_bihar, q5perf_weighted_mean_chandigarh, q5perf_weighted_mean_chhattisgarh, q5perf_weighted_mean_delhi, q5perf_weighted_mean_dnhdd, q5perf_weighted_mean_goa, q5perf_weighted_mean_gujarat, q5perf_weighted_mean_haryana, q5perf_weighted_mean_hp, q5perf_weighted_mean_jharkhand, q5perf_weighted_mean_jk, q5perf_weighted_mean_karnataka, q5perf_weighted_mean_kerala, q5perf_weighted_mean_ladakh, q5perf_weighted_mean_lakshadweep, q5perf_weighted_mean_maharashtra, q5perf_weighted_mean_manipur, q5perf_weighted_mean_meghalaya, q5perf_weighted_mean_mizoram, q5perf_weighted_mean_mp, q5perf_weighted_mean_nagaland, q5perf_weighted_mean_none, q5perf_weighted_mean_odisha, q5perf_weighted_mean_puducherry, q5perf_weighted_mean_punjab, q5perf_weighted_mean_rajasthan, q5perf_weighted_mean_sikkim, q5perf_weighted_mean_telangana, q5perf_weighted_mean_tn, q5perf_weighted_mean_tripura, q5perf_weighted_mean_up, q5perf_weighted_mean_uttarakhand, q5perf_weighted_mean_wb)

q5frnt_plot1 <- c(q5frnt_weighted_mean_anislands, q5frnt_weighted_mean_andhra, q5frnt_weighted_mean_arunachal, q5frnt_weighted_mean_assam, q5frnt_weighted_mean_bihar, q5frnt_weighted_mean_chandigarh, q5frnt_weighted_mean_chhattisgarh, q5frnt_weighted_mean_delhi, q5frnt_weighted_mean_dnhdd, q5frnt_weighted_mean_goa, q5frnt_weighted_mean_gujarat, q5frnt_weighted_mean_haryana, q5frnt_weighted_mean_hp, q5frnt_weighted_mean_jharkhand, q5frnt_weighted_mean_jk, q5frnt_weighted_mean_karnataka, q5frnt_weighted_mean_kerala, q5frnt_weighted_mean_ladakh, q5frnt_weighted_mean_lakshadweep, q5frnt_weighted_mean_maharashtra, q5frnt_weighted_mean_manipur, q5frnt_weighted_mean_meghalaya, q5frnt_weighted_mean_mizoram, q5frnt_weighted_mean_mp, q5frnt_weighted_mean_nagaland, q5frnt_weighted_mean_none, q5frnt_weighted_mean_odisha, q5frnt_weighted_mean_puducherry, q5frnt_weighted_mean_punjab, q5frnt_weighted_mean_rajasthan, q5frnt_weighted_mean_sikkim, q5frnt_weighted_mean_telangana, q5frnt_weighted_mean_tn, q5frnt_weighted_mean_tripura, q5frnt_weighted_mean_up, q5frnt_weighted_mean_uttarakhand, q5frnt_weighted_mean_wb)

q5aspt_plot_dta1 <- data.frame (States_Q5, q5aspt_plot1) 
q5perf_plot_dta1 <- data.frame (q5perf_plot1)
q5frnt_plot_dta1 <- data.frame (q5frnt_plot1)

q5_sdg_dta <- cbind (q5aspt_plot_dta1, q5perf_plot_dta1, q5frnt_plot_dta1) %>% 
arrange (desc(q5perf_plot_dta1)) %>% 
rename ("Percentages (Q5 Aspirant)" = q5aspt_plot1) %>% 
rename ("Percentages (Q5 Performer)" = q5perf_plot1) %>% 
rename ("Percentages (Q5 Frontrunner)" = q5frnt_plot1)

write.table (q5_plot_dta1, "/Users/gurkirat/Documents/GitHub/ias-release/q5_plot_dta1.txt", sep = "\t")
write.table (q5_age_dta, "/Users/gurkirat/Documents/GitHub/ias-release/q5_age_dta.txt", sep = "\t")
write.table (q5_sdg_dta, "/Users/gurkirat/Documents/GitHub/ias-release/q5_sdg_dta.txt", sep = "\t")
```

```{r}
#Tables for question 6
`States_Q6` <- c("Andaman and Nicobar Islands", "Andhra Pradesh", "Arunachal Pradesh", "Assam", "Bihar", "Chandigarh", "Chattisgarh", "NCT of Delhi","Dadra and Nagar Haveli and Daman and Dui", "Goa", "Gujarat", "Haryana", "Himachal Pradesh", "Jharkhand", "Jammu and Kashmir", "Karnataka", "Kerala", "Ladakh", "Lakshadweep Islands", "Maharashtra", "Manipur", "Meghalaya", "Mizoram", "Madhya Pradesh", "Nagaland","No state has been under-appreciated", "Odisha", "Puducherry", "Punjab", "Rajasthan", "Sikkim", "Telangana", "Tamil Nadu", "Tripura", "Uttar Pradesh", "Uttarakhand", "West Bengal")

#Question 6, aggregate
q6full_plot1 <- c(q6full_mean_anislands, q6full_mean_andhra, q6full_mean_arunachal, q6full_mean_assam, q6full_mean_bihar, q6full_mean_chandigarh, q6full_mean_chhattisgarh, q6full_mean_delhi, q6full_mean_dnhdd, q6full_mean_goa, q6full_mean_gujarat, q6full_mean_haryana, q6full_mean_hp, q6full_mean_jharkhand, q6full_mean_jk, q6full_mean_karnataka, q6full_mean_kerala, q6full_mean_ladakh, q6full_mean_lakshadweep, q6full_mean_maharashtra, q6full_mean_manipur, q6full_mean_meghalaya, q6full_mean_mizoram, q6full_mean_mp, q6full_mean_nagaland, q6full_mean_none, q6full_mean_odisha, q6full_mean_puducherry, q6full_mean_punjab, q6full_mean_rajasthan, q6full_mean_sikkim, q6full_mean_telangana, q6full_mean_tn, q6full_mean_tripura, q6full_mean_up, q6full_mean_uttarakhand, q6full_mean_wb)

q6_plot_dta1 <- data.frame (States_Q6, q6full_plot1) %>% 
arrange (desc(q6full_plot1)) %>% 
rename ("Aggregate Percentages (Q6)" = q6full_plot1)

#Question 6, aggregate (accounting for officer's bias)
q6full_plot2 <- c(q6full_mean_anislands, q6full_mean_andhra, q6full_mean_arunachal, q6full_mean_assam, q6full_mean_bihar, q6full_mean_chandigarh, q6full_mean_chhattisgarh, q6full_mean_delhi, q6full_mean_dnhdd, q6full_mean_goa, q6full_mean_gujarat, q6full_mean_haryana, q6full_mean_hp, q6full_mean_jharkhand, q6full_mean_jk, q6full_mean_karnataka, q6full_mean_kerala, q6full_mean_ladakh, q6full_mean_lakshadweep, q6full_mean_maharashtra, q6full_mean_manipur, q6full_mean_meghalaya, q6full_mean_mizoram, q6full_mean_mp, q6full_mean_nagaland, q6full_mean_none, q6full_mean_odisha, q6full_mean_puducherry, q6full_mean_punjab, q6full_mean_rajasthan, q6full_mean_sikkim, q6full_mean_telangana, q6full_mean_tn, q6full_mean_tripura, q6full_mean_up, q6full_mean_uttarakhand, q6full_mean_wb)

q6_plot_dta2 <- data.frame (States_Q6, q6full_plot2) %>% 
arrange (desc(q6full_plot2)) %>% 
rename ("Q6 Aggregate Percentages (accounting for officer's bias)" = q6full_plot2)

#Question 6, Disaggreaged by Age
q6young_plot1 <- c(q6young_mean_anislands, q6young_mean_andhra, q6young_mean_arunachal, q6young_mean_assam, q6young_mean_bihar, q6young_mean_chandigarh, q6young_mean_chhattisgarh, q6young_mean_delhi, q6young_mean_dnhdd, q6young_mean_goa, q6young_mean_gujarat, q6young_mean_haryana, q6young_mean_hp, q6young_mean_jharkhand, q6young_mean_jk, q6young_mean_karnataka, q6young_mean_kerala, q6young_mean_ladakh, q6young_mean_lakshadweep, q6young_mean_maharashtra, q6young_mean_manipur, q6young_mean_meghalaya, q6young_mean_mizoram, q6young_mean_mp, q6young_mean_nagaland, q6young_mean_none, q6young_mean_odisha, q6young_mean_puducherry, q6young_mean_punjab, q6young_mean_rajasthan, q6young_mean_sikkim, q6young_mean_telangana, q6young_mean_tn, q6young_mean_tripura, q6young_mean_up, q6young_mean_uttarakhand, q6young_mean_wb)

q6young_plot_dta1 <- data.frame (States_Q6, q6young_plot1) 

q6old_plot1 <- c(q6old_mean_anislands, q6old_mean_andhra, q6old_mean_arunachal, q6old_mean_assam, q6old_mean_bihar, q6old_mean_chandigarh, q6old_mean_chhattisgarh, q6old_mean_delhi, q6old_mean_dnhdd, q6old_mean_goa, q6old_mean_gujarat, q6old_mean_haryana, q6old_mean_hp, q6old_mean_jharkhand, q6old_mean_jk, q6old_mean_karnataka, q6old_mean_kerala, q6old_mean_ladakh, q6old_mean_lakshadweep, q6old_mean_maharashtra, q6old_mean_manipur, q6old_mean_meghalaya, q6old_mean_mizoram, q6old_mean_mp, q6old_mean_nagaland, q6old_mean_none, q6old_mean_odisha, q6old_mean_puducherry, q6old_mean_punjab, q6old_mean_rajasthan, q6old_mean_sikkim, q6old_mean_telangana, q6old_mean_tn, q6old_mean_tripura, q6old_mean_up, q6old_mean_uttarakhand, q6old_mean_wb)

q6old_plot_dta1 <- data.frame (q6old_plot1)

q6former_plot1 <- c(q6former_mean_anislands, q6former_mean_andhra, q6former_mean_arunachal, q6former_mean_assam, q6former_mean_bihar, q6former_mean_chandigarh, q6former_mean_chhattisgarh, q6former_mean_delhi, q6former_mean_dnhdd, q6former_mean_goa, q6former_mean_gujarat, q6former_mean_haryana, q6former_mean_hp, q6former_mean_jharkhand, q6former_mean_jk, q6former_mean_karnataka, q6former_mean_kerala, q6former_mean_ladakh, q6former_mean_lakshadweep, q6former_mean_maharashtra, q6former_mean_manipur, q6former_mean_meghalaya, q6former_mean_mizoram, q6former_mean_mp, q6former_mean_nagaland, q6former_mean_none, q6former_mean_odisha, q6former_mean_puducherry, q6former_mean_punjab, q6former_mean_rajasthan, q6former_mean_sikkim, q6former_mean_telangana, q6former_mean_tn, q6former_mean_tripura, q6former_mean_up, q6former_mean_uttarakhand, q6former_mean_wb)

q6former_plot_dta1 <- data.frame (q6former_plot1)

q6_age_dta <- cbind (q6young_plot_dta1, q6old_plot_dta1, q6former_plot_dta1) %>% 
arrange (desc(q6old_plot_dta1)) %>% 
rename ("Percentages (Q6 Young)" = q6young_plot1) %>% 
rename ("Percentages (Q6 Old)" = q6old_plot1) %>% 
rename ("Percentages (Q6 Former)" = q6former_plot1)

#Question 6, Disaggregated by Age (accounting for officer's bias)
q6young_plot2 <- c(q6young_mean_anislands, q6young_mean_andhra, q6young_mean_arunachal, q6young_mean_assam, q6young_mean_bihar, q6young_mean_chandigarh, q6young_mean_chhattisgarh, q6young_mean_delhi, q6young_mean_dnhdd, q6young_mean_goa, q6young_mean_gujarat, q6young_mean_haryana, q6young_mean_hp, q6young_mean_jharkhand, q6young_mean_jk, q6young_mean_karnataka, q6young_mean_kerala, q6young_mean_ladakh, q6young_mean_lakshadweep, q6young_mean_maharashtra, q6young_mean_manipur, q6young_mean_meghalaya, q6young_mean_mizoram, q6young_mean_mp, q6young_mean_nagaland, q6young_mean_none, q6young_mean_odisha, q6young_mean_puducherry, q6young_mean_punjab, q6young_mean_rajasthan, q6young_mean_sikkim, q6young_mean_telangana, q6young_mean_tn, q6young_mean_tripura, q6young_mean_up, q6young_mean_uttarakhand, q6young_mean_wb)

q6old_plot2 <- c(q6old_mean_anislands, q6old_mean_andhra, q6old_mean_arunachal, q6old_mean_assam, q6old_mean_bihar, q6old_mean_chandigarh, q6old_mean_chhattisgarh, q6old_mean_delhi, q6old_mean_dnhdd, q6old_mean_goa, q6old_mean_gujarat, q6old_mean_haryana, q6old_mean_hp, q6old_mean_jharkhand, q6old_mean_jk, q6old_mean_karnataka, q6old_mean_kerala, q6old_mean_ladakh, q6old_mean_lakshadweep, q6old_mean_maharashtra, q6old_mean_manipur, q6old_mean_meghalaya, q6old_mean_mizoram, q6old_mean_mp, q6old_mean_nagaland, q6old_mean_none, q6old_mean_odisha, q6old_mean_puducherry, q6old_mean_punjab, q6old_mean_rajasthan, q6old_mean_sikkim, q6old_mean_telangana, q6old_mean_tn, q6old_mean_tripura, q6old_mean_up, q6old_mean_uttarakhand, q6old_mean_wb)

q6former_plot2 <- c(q6former_mean_anislands, q6former_mean_andhra, q6former_mean_arunachal, q6former_mean_assam, q6former_mean_bihar, q6former_mean_chandigarh, q6former_mean_chhattisgarh, q6former_mean_delhi, q6former_mean_dnhdd, q6former_mean_goa, q6former_mean_gujarat, q6former_mean_haryana, q6former_mean_hp, q6former_mean_jharkhand, q6former_mean_jk, q6former_mean_karnataka, q6former_mean_kerala, q6former_mean_ladakh, q6former_mean_lakshadweep, q6former_mean_maharashtra, q6former_mean_manipur, q6former_mean_meghalaya, q6former_mean_mizoram, q6former_mean_mp, q6former_mean_nagaland, q6former_mean_none, q6former_mean_odisha, q6former_mean_puducherry, q6former_mean_punjab, q6former_mean_rajasthan, q6former_mean_sikkim, q6former_mean_telangana, q6former_mean_tn, q6former_mean_tripura, q6former_mean_up, q6former_mean_uttarakhand, q6former_mean_wb)

q6young_plot_dta2 <- data.frame (States_Q6, q6young_plot2) 
q6old_plot_dta2 <- data.frame (q6old_plot2)
q6former_plot_dta2 <- data.frame (q6former_plot2)

q6_age_dta2 <- cbind (q6young_plot_dta2, q6old_plot_dta2, q6former_plot_dta2) %>% 
arrange (desc(q6old_plot_dta2)) %>% 
rename ("Q6 Percentages (Young, Unbiased)" = q6young_plot2) %>% 
rename ("Q6 Percentages (Old, Unbiased)" = q6old_plot2) %>% 
rename ("Q6 Percentages (Former, Unbiased)" = q6former_plot2)

#Question 6, Disaggregated by SDG
q6aspt_plot1 <- c(q6aspt_weighted_mean_anislands, q6aspt_weighted_mean_andhra, q6aspt_weighted_mean_arunachal, q6aspt_weighted_mean_assam, q6aspt_weighted_mean_bihar, q6aspt_weighted_mean_chandigarh, q6aspt_weighted_mean_chhattisgarh, q6aspt_weighted_mean_delhi, q6aspt_weighted_mean_dnhdd, q6aspt_weighted_mean_goa, q6aspt_weighted_mean_gujarat, q6aspt_weighted_mean_haryana, q6aspt_weighted_mean_hp, q6aspt_weighted_mean_jharkhand, q6aspt_weighted_mean_jk, q6aspt_weighted_mean_karnataka, q6aspt_weighted_mean_kerala, q6aspt_weighted_mean_ladakh, q6aspt_weighted_mean_lakshadweep, q6aspt_weighted_mean_maharashtra, q6aspt_weighted_mean_manipur, q6aspt_weighted_mean_meghalaya, q6aspt_weighted_mean_mizoram, q6aspt_weighted_mean_mp, q6aspt_weighted_mean_nagaland, q6aspt_weighted_mean_none, q6aspt_weighted_mean_odisha, q6aspt_weighted_mean_puducherry, q6aspt_weighted_mean_punjab, q6aspt_weighted_mean_rajasthan, q6aspt_weighted_mean_sikkim, q6aspt_weighted_mean_telangana, q6aspt_weighted_mean_tn, q6aspt_weighted_mean_tripura, q6aspt_weighted_mean_up, q6aspt_weighted_mean_uttarakhand, q6aspt_weighted_mean_wb)

q6perf_plot1 <- c(q6perf_weighted_mean_anislands, q6perf_weighted_mean_andhra, q6perf_weighted_mean_arunachal, q6perf_weighted_mean_assam, q6perf_weighted_mean_bihar, q6perf_weighted_mean_chandigarh, q6perf_weighted_mean_chhattisgarh, q6perf_weighted_mean_delhi, q6perf_weighted_mean_dnhdd, q6perf_weighted_mean_goa, q6perf_weighted_mean_gujarat, q6perf_weighted_mean_haryana, q6perf_weighted_mean_hp, q6perf_weighted_mean_jharkhand, q6perf_weighted_mean_jk, q6perf_weighted_mean_karnataka, q6perf_weighted_mean_kerala, q6perf_weighted_mean_ladakh, q6perf_weighted_mean_lakshadweep, q6perf_weighted_mean_maharashtra, q6perf_weighted_mean_manipur, q6perf_weighted_mean_meghalaya, q6perf_weighted_mean_mizoram, q6perf_weighted_mean_mp, q6perf_weighted_mean_nagaland, q6perf_weighted_mean_none, q6perf_weighted_mean_odisha, q6perf_weighted_mean_puducherry, q6perf_weighted_mean_punjab, q6perf_weighted_mean_rajasthan, q6perf_weighted_mean_sikkim, q6perf_weighted_mean_telangana, q6perf_weighted_mean_tn, q6perf_weighted_mean_tripura, q6perf_weighted_mean_up, q6perf_weighted_mean_uttarakhand, q6perf_weighted_mean_wb)

q6frnt_plot1 <- c(q6frnt_weighted_mean_anislands, q6frnt_weighted_mean_andhra, q6frnt_weighted_mean_arunachal, q6frnt_weighted_mean_assam, q6frnt_weighted_mean_bihar, q6frnt_weighted_mean_chandigarh, q6frnt_weighted_mean_chhattisgarh, q6frnt_weighted_mean_delhi, q6frnt_weighted_mean_dnhdd, q6frnt_weighted_mean_goa, q6frnt_weighted_mean_gujarat, q6frnt_weighted_mean_haryana, q6frnt_weighted_mean_hp, q6frnt_weighted_mean_jharkhand, q6frnt_weighted_mean_jk, q6frnt_weighted_mean_karnataka, q6frnt_weighted_mean_kerala, q6frnt_weighted_mean_ladakh, q6frnt_weighted_mean_lakshadweep, q6frnt_weighted_mean_maharashtra, q6frnt_weighted_mean_manipur, q6frnt_weighted_mean_meghalaya, q6frnt_weighted_mean_mizoram, q6frnt_weighted_mean_mp, q6frnt_weighted_mean_nagaland, q6frnt_weighted_mean_none, q6frnt_weighted_mean_odisha, q6frnt_weighted_mean_puducherry, q6frnt_weighted_mean_punjab, q6frnt_weighted_mean_rajasthan, q6frnt_weighted_mean_sikkim, q6frnt_weighted_mean_telangana, q6frnt_weighted_mean_tn, q6frnt_weighted_mean_tripura, q6frnt_weighted_mean_up, q6frnt_weighted_mean_uttarakhand, q6frnt_weighted_mean_wb)

q6aspt_plot_dta1 <- data.frame (States_Q6, q6aspt_plot1) 
q6perf_plot_dta1 <- data.frame (q6perf_plot1)
q6frnt_plot_dta1 <- data.frame (q6frnt_plot1)

q6_sdg_dta <- cbind (q6aspt_plot_dta1, q6perf_plot_dta1, q6frnt_plot_dta1) %>% 
arrange (desc(q6perf_plot_dta1)) %>% 
rename ("Percentages (Q6 Aspirant)" = q6aspt_plot1) %>% 
rename ("Percentages (Q6 Performer)" = q6perf_plot1) %>% 
rename ("Percentages (Q6 Frontrunner)" = q6frnt_plot1)

#Question 6: Disaggregated by SDG (accounting for officer's bias)
q6aspt_plot2 <- c(q6aspt_weighted_mean_anislands, q6aspt_weighted_mean_andhra, q6aspt_weighted_mean_arunachal, q6aspt_weighted_mean_assam, q6aspt_weighted_mean_bihar, q6aspt_weighted_mean_chandigarh, q6aspt_weighted_mean_chhattisgarh, q6aspt_weighted_mean_delhi, q6aspt_weighted_mean_dnhdd, q6aspt_weighted_mean_goa, q6aspt_weighted_mean_gujarat, q6aspt_weighted_mean_haryana, q6aspt_weighted_mean_hp, q6aspt_weighted_mean_jharkhand, q6aspt_weighted_mean_jk, q6aspt_weighted_mean_karnataka, q6aspt_weighted_mean_kerala, q6aspt_weighted_mean_ladakh, q6aspt_weighted_mean_lakshadweep, q6aspt_weighted_mean_maharashtra, q6aspt_weighted_mean_manipur, q6aspt_weighted_mean_meghalaya, q6aspt_weighted_mean_mizoram, q6aspt_weighted_mean_mp, q6aspt_weighted_mean_nagaland, q6aspt_weighted_mean_none, q6aspt_weighted_mean_odisha, q6aspt_weighted_mean_puducherry, q6aspt_weighted_mean_punjab, q6aspt_weighted_mean_rajasthan, q6aspt_weighted_mean_sikkim, q6aspt_weighted_mean_telangana, q6aspt_weighted_mean_tn, q6aspt_weighted_mean_tripura, q6aspt_weighted_mean_up, q6aspt_weighted_mean_uttarakhand, q6aspt_weighted_mean_wb)

q6perf_plot2 <- c(q6perf_weighted_mean_anislands, q6perf_weighted_mean_andhra, q6perf_weighted_mean_arunachal, q6perf_weighted_mean_assam, q6perf_weighted_mean_bihar, q6perf_weighted_mean_chandigarh, q6perf_weighted_mean_chhattisgarh, q6perf_weighted_mean_delhi, q6perf_weighted_mean_dnhdd, q6perf_weighted_mean_goa, q6perf_weighted_mean_gujarat, q6perf_weighted_mean_haryana, q6perf_weighted_mean_hp, q6perf_weighted_mean_jharkhand, q6perf_weighted_mean_jk, q6perf_weighted_mean_karnataka, q6perf_weighted_mean_kerala, q6perf_weighted_mean_ladakh, q6perf_weighted_mean_lakshadweep, q6perf_weighted_mean_maharashtra, q6perf_weighted_mean_manipur, q6perf_weighted_mean_meghalaya, q6perf_weighted_mean_mizoram, q6perf_weighted_mean_mp, q6perf_weighted_mean_nagaland, q6perf_weighted_mean_none, q6perf_weighted_mean_odisha, q6perf_weighted_mean_puducherry, q6perf_weighted_mean_punjab, q6perf_weighted_mean_rajasthan, q6perf_weighted_mean_sikkim, q6perf_weighted_mean_telangana, q6perf_weighted_mean_tn, q6perf_weighted_mean_tripura, q6perf_weighted_mean_up, q6perf_weighted_mean_uttarakhand, q6perf_weighted_mean_wb)

q6frnt_plot2 <- c(q6frnt_weighted_mean_anislands, q6frnt_weighted_mean_andhra, q6frnt_weighted_mean_arunachal, q6frnt_weighted_mean_assam, q6frnt_weighted_mean_bihar, q6frnt_weighted_mean_chandigarh, q6frnt_weighted_mean_chhattisgarh, q6frnt_weighted_mean_delhi, q6frnt_weighted_mean_dnhdd, q6frnt_weighted_mean_goa, q6frnt_weighted_mean_gujarat, q6frnt_weighted_mean_haryana, q6frnt_weighted_mean_hp, q6frnt_weighted_mean_jharkhand, q6frnt_weighted_mean_jk, q6frnt_weighted_mean_karnataka, q6frnt_weighted_mean_kerala, q6frnt_weighted_mean_ladakh, q6frnt_weighted_mean_lakshadweep, q6frnt_weighted_mean_maharashtra, q6frnt_weighted_mean_manipur, q6frnt_weighted_mean_meghalaya, q6frnt_weighted_mean_mizoram, q6frnt_weighted_mean_mp, q6frnt_weighted_mean_nagaland, q6frnt_weighted_mean_none, q6frnt_weighted_mean_odisha, q6frnt_weighted_mean_puducherry, q6frnt_weighted_mean_punjab, q6frnt_weighted_mean_rajasthan, q6frnt_weighted_mean_sikkim, q6frnt_weighted_mean_telangana, q6frnt_weighted_mean_tn, q6frnt_weighted_mean_tripura, q6frnt_weighted_mean_up, q6frnt_weighted_mean_uttarakhand, q6frnt_weighted_mean_wb)

q6aspt_plot_dta2 <- data.frame (States_Q6, q6aspt_plot2) 
q6perf_plot_dta2 <- data.frame (q6perf_plot2)
q6frnt_plot_dta2 <- data.frame (q6frnt_plot2)

q6_sdg_dta2 <- cbind (q6aspt_plot_dta2, q6perf_plot_dta2, q6frnt_plot_dta2) %>% 
arrange (desc(q6perf_plot_dta2)) %>% 
rename ("Q6 Percentages (Aspirant, Unbiased)" = q6aspt_plot2) %>% 
rename ("Q6 Percentages (Performer, Unbiased)" = q6perf_plot2) %>% 
rename ("Q6 Percentages (Frontrunner, Unbiased)" = q6frnt_plot2)


write.table (q6_plot_dta1, "/Users/gurkirat/Documents/GitHub/ias-release/q6_plot_dta1.txt", sep = "\t")
write.table (q6_age_dta, "/Users/gurkirat/Documents/GitHub/ias-release/q6_age_dta.txt", sep = "\t")
write.table (q6_sdg_dta, "/Users/gurkirat/Documents/GitHub/ias-release/q6_sdg_dta.txt", sep = "\t")
write.table (q6_unbiased, "/Users/gurkirat/Documents/GitHub/ias-release/q6_unbiased.txt", sep = "\t")
write.table (q6_plot_dta2, "/Users/gurkirat/Documents/GitHub/ias-release/q6_plot_dta2.txt", sep = "\t")
write.table (q6_age_dta2, "/Users/gurkirat/Documents/GitHub/ias-release/q6_age_dta2.txt", sep = "\t")
write.table (q6_sdg_dta2, "/Users/gurkirat/Documents/GitHub/ias-release/q6_sdg_dta2.txt", sep = "\t")
```
