...
#Leaving Q19 (X136) as is since this is the error question #delete the question
...
#let's get rid of this - only do the character to numeric once for the entire data and then split things out. so let's do this before we create full_serving 
```{r, include=FALSE}
...
#Rahul says: I'm not clear about what's going on in these state and sdg chunks at the bottom of the code. We don't need to compute a new average using the length of sdg or age (not sure what those are). Let's talk about what you are thinking here. All we need are the averages for each answer by STATE. Then we multiply that by SDG weights. 
