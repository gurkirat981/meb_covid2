...
```{r}
#Question 4:Averages for the aggregate graph (by omitting responses identical to the cadre)

q4_non_repeated <- full_serving %>% 
  dplyr::select (state, X12, Merged1.1, X12) %>% 
  mutate (Merged = Merged1.1) %>% 
  dplyr:: select(-Merged1.1)

non_repeated_q4 <- full_serving %>% 
  dplyr::select (state, X12, Merged1.2, X12) %>% 
  mutate (Merged = Merged1.2) %>% 
  dplyr:: select (-Merged1.2) %>% 
  add_row(q4_non_repeated) 

non_repeated_q4 <- non_repeated_q4 %>% 
...
```{r, include=FALSE}
#For young officers only: calculating averages for question 4 (by omitting responses identical to the cadre)
q4_state_answers1_young <- fullage_young %>% 
...
```{r, include=FALSE}
#For old officers only: calculating averages for question 4 (by omitting responses identical to the cadre)
q4_state_answers1_old <- fullage_old %>% 
  dplyr::select (state, Merged1.1, X12) %>% 
  mutate (Merged = Merged1.1) %>% 
  dplyr:: select(-Merged1.1)
...
```{r, include=FALSE}
#For former officers only: calculating averages for question 4 (by omitting responses identical to the cadre)
q4_state_answers1_former <- full_former %>% 
...
## Question 4: Aggregate Graph (by omitting responses identical to the cadre)
...
## Question 4: Disaggregated by Age (by omitting responses identical to the cadre)
...
## Question 4: Disaggregated by SDG (by omitting responses identical to the cadre)
