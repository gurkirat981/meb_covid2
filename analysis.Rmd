...
#Rahul says: the probabilities need to be corrected again. Rememeber that the probs all need to add upto 1. So prob = c(1.0,0.5, 0.4, 0.3, 0.2, 0.1, 0.1, 0.1, 0.1) is not correct. If you refer to the rough excel sheet I had shared, you will simply put down the proportions as probabilities. So Delhi is 0.36, Arun Pradesh is 0.18 etc. 
...
#Rahul says: my understanding was that we had decided to create a single code chunk with all the mutations we needed (such as the "age" column created above). I'll send you my code over email so you can take a look. Then let's have a separate set of code of making subsets.
...
```{r}
#Let this code chunk be called "creating subsets" and do all subsetting here. 
```
...
#Rahul says: minor thing, but be precise in your coding notes. These are not "labels". You are converting character strings into numerics. Just say that instead of calling them labels. Labels are often additional meta-data you add to an existing piece of information. 

#Criteria for attaching labels: Have created additional columns with labeled values (feel free to delete the corresponding non-labeled columns if they are not needed anymore)

#Rahul says: not sure I understand your comment above about creating additional columns. From the code below it seems to me that you have mutated the original column itself right? Second, your coding format below is good, but always indent any new ifelse statement like I've done below. You simply need to hit enter to do that. 
...
mutate (X139 = ifelse (X139 == "By focussing on failures in pandemic management, the media has exaggerated the sense of failure and despair.", 1, 
                       ifelse(X133 == "The media has failed to capture the full scale and intensity of the disaster.", 2, 3))) %>% 

#Question 19 (not certain)
mutate (X136 = ifelse (X136 == "The government must acknowledge its failure.", 0, 1))

##Gurkirat##
```


Rahul says: the code for converting character responses to numeric values looks good. Please combine the two code chunks you have worked on separately now into a single code chunk and remove your names from them. This code chunk will start with a comment that clearly states that in the following code, we can convereted all character strings into numeric values for ease of calculating average responses later on. 
